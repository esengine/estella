...
- A native Android app can reach the network: its manifest never asked for it, so a remote group's assets, hot updates and the debug channel all failed on Android. A development build also allows plain http (a LAN CDN); a shipping build is held to https, and the export warns when its CDN is http.
...
- **A project can be packaged as a Huawei quick game (华为快游戏).** The same signed `.rpk` as the quick-game alliance — Huawei's signtool writes the same block, and ours matches it byte for byte — with Huawei's manifest (`appType: "fastgame"`, orientation under `display`) and its own package name, version code and release key in Project Settings. It packages the engine build without SIMD and is judged against 4 MB and the 20 MB AppGallery Connect takes. Subpackages are refused rather than packaged wrongly (Huawei packs each as its own `.rpk`); sign-in, purchase and share go through HMS Core with shapes of their own and are not offered. Huawei's loader asks Huawei's servers about a package before it runs one, so nothing has run without an app registered in AppGallery Connect.
- **A project can be packaged for Alipay (支付宝小游戏).** `game.json` turns on iOS high-performance mode — the only iOS mode with `MYWebAssembly`, which the runtime instantiates through with an absolute package path — and `project.config.json` turns off the review build's own transpile and minify, since the bundle is already down-levelled; the appid is passed to Alipay's tool (`minidev -a`), not packaged. The profile hands the family `my` in WeChat's words — a rewarded unit through `my.createRewardedAd`, sign-in through `my.getAuthCode`, share through the card `my.showSharePanel` reads off `my.onShareAppMessage`. It packages the engine build without SIMD, judged against 4 MB and 20 MB. Nothing has run on a device; purchase is not offered (Android and Harmony only, keyed by a `customId`), and Taobao — which only supports Cocos and Laya — is not a target.
- **A project can be packaged as a quick game — one signed `.rpk` for vivo, OPPO, Xiaomi and Honor (快游戏联盟).** Set the package name (it must end in `.minigame`) in Project Settings → Packaging → Quick game, and the export writes `manifest.json` (allianceVersion 1300), the project's icon (or Estella's mark), and `<package>.rpk` signed with your release key — or with Estella's public debug key, which a device debugger installs and a store refuses. It needs no vendor tool: the signing block is written here and matches the vendor CLI's byte for byte. The package runs on its own engine build (`build -t quickgame`) without wasm SIMD and BigInt i64, which the hosts' V8 8.3 refuses, and the runtime reads the hosts' window in physical pixels and turns WebAssembly on first. Verified end to end on vivo's engine in an Android emulator; OPPO grants WebGL2 only to a game that applied for it, and no device has run it yet.
- **A project can be packaged for Bilibili (B 站).** `game.json` carries the appid (Project Settings → Packaging → Bilibili) and the project's version, and turns on all four high-performance switches — Bilibili runs WebGL2 nowhere else, and the engine renders on nothing less. Scripts are lowered below async/await, which Bilibili asks a game to polyfill, and files are staged against Bilibili's own upload whitelist. The build is judged against 4 MB for the main package, 4 MB for each subpackage and 30 MB in all; a per-subpackage limit is new, and the size panel names the heaviest subpackage's files when it is over. Ads (rewarded only), share and sign-in are forwarded to `bl` and marked unverified; purchase is not offered. wasm on iOS is contradicted in Bilibili's own docs and needs a device.
- A mini-game host that asks to hear when the first screen has rendered is told, once, at the end of the first frame — Bilibili reviews a game by `bl.launchSuccess`.
- **A project can be packaged for Kuaishou (快手).** The third mini-game vendor rides the same pipeline and runtime as WeChat and Douyin: a package of `game.js` + `game.json` (the appid is entered in Kuaishou's developer tool, not written into the package), judged against Kuaishou's 6 MB main package and 30 MB total. The recorder shares through Kuaishou's own `publishVideo`, whole — the host takes no ranges. Ads, share and sign-in are forwarded to `ks` and marked unverified; purchase is not offered, since `ks.requestGamePayment` needs an order signed by your server. It renders only where the host offers WebGL2, which Kuaishou still calls beta.
...
- **`tsc` and an outside editor type-check the official packages too.** Opening a project mirrors the packages the editor ships into `.esengine/packages/`, beside the SDK's types, and a tsconfig maps them with `"estella-plugin-*": ["./.esengine/packages/*"]` — every template and example now carries it, and opening a project whose tsconfig is the shape Estella wrote adds the line when it is missing.
...
- **Retargeting works: one character's clips play on another's skeleton.** A controller's `avatar` — the rig its clips were authored on — was never loaded, so no retarget ever ran and a second rig played the clips' raw values. It is loaded with the controller now, and the retarget carries each joint's turn through the two rigs' own spaces, so skeletons that stand on different axes (one Z-up under a turned root, one Y-up) read a clip alike, and a clip that moves the hips moves them from the target's own rest, scaled by the rigs' sizes. An avatar states this in two new optional fields, `space` and `restPosition`.
- **A prefab instance's override that names no entity of its prefab is reported.** It was applied nowhere and said nothing; loading a scene now warns which instance and which entity, and `check-prefab-overrides` holds every shipped scene to it.
- **The `character-rig` example shows what it is for.** Its hero's position and Animator were overridden on an entity the prefab does not have, so the hero stood at the origin in its bind pose; and without the retarget its knight lay on its back with its head at the camera. Both now stand side by side, both animated, and the camera frames them.
- **A hard shadow's edge is a clean line, not a row of stair-steps.** Its four samples of the shadow map were weighted equally, so the edge moved in whole texels of the map, and wherever the shadow crossed the map's grid at an angle — which is almost everywhere — its edge showed the grid as dotted steps. It is now filtered over the 3×3 texels around each point, weighted by where it falls among them: on a shadow at 30° to the map the edge strays 0.05 px from a straight line, where it strayed 0.25.
- **A drawn sky is the panorama you imported, not a blur of it.** The sky was drawn from the environment's reflection atlas — the whole sphere in 128 texels a side, laid out so a straight horizon became a staircase and a round sun an oval, and filtered in its encoded form, which blotted every sharp edge. Importing a `.hdr` now also writes the panorama at its own resolution (up to 2048 wide; `skyWidth` in the import settings, 0 for none), the sky is drawn from it, and a package carries it. Environments imported before draw their sky from the atlas as they did. The `lighting-3d` example draws its sky, from a panorama generated at 1024×512.
- **A drawn sky no longer vanishes, or half-vanishes, as the view turns.** The sky was a quad on the far plane, and where rounding put it a hair past that plane it was clipped: in the editor, with its distant far plane, more than half the angles showed black behind the grid, or a sky cut along a diagonal. It now sits inside the view volume and fills the frame from every angle, in the editor and in the game.
...
- A mini-game that fails to start says why. The generated entry now catches a boot that rejects — a host without `onUnhandledRejection` dropped it silently, leaving the loading screen up and nothing in the log — and a failed engine instantiation names the error instead of printing `[object Object]`.
- `estella export --platform <id>` packages a platform the project defines in `.esengine/platforms/`, as the build dialog does, and an id that is neither built in nor defined is refused instead of silently packaged as a web game.
- A mini-game host that offers only WebGL1 is told the game cannot start there, instead of booting to shader failures and a black screen — the engine renders on WebGL2.
...
- **Pressing Play could kill the session it was starting.** The editor hands the scene to the realm from two doors — directly on a warm re-Play, and on the realm's greeting when it cold-boots — and a greeting that lands while `start()` is still rebuilding scripts goes through both. The realm read the second hand-over as a re-Play and quit the App the first was still booting on, which surfaced a few steps later as a null read and a session that never reached a frame. It now ignores a hand-over that arrives during its own boot; both are still posted, because the first is lost often enough that dropping either strands the realm.
...
- **Nine import settings did nothing.** The asset inspector offered Premultiply
  ...
  armature and animation — every one of them written into fresh `.meta` files and
  read by nothing. Six were a second copy of a component field that already
  decides the thing (a Video's playback, a Spine skin, a DragonBones armature);
  migration happens on load whatever its flag says; premultiplied alpha was never
  a path the texture upload had. They are gone, and a scene or prefab now has no
  import settings at all, which is the truth about an authored asset. Found by a
  new gate — the sibling of the one that checks component fields, one layer out:
  a declared import setting must have a reader, or be recorded as a gap saying
  what is missing.
