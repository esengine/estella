...
export function emitC(
    ...
    /** `sizeof(es_addr_t)` where this artifact will run: 4 for a host owning one
     *  block it addresses by offset, 8 where an address is a pointer. The machine
     *  and not the contract, so it is mixed into the handshake and not into the
     *  offsets — the digest is what refuses a 32-bit module to a 64-bit host. */
    addressBytes: 4 | 8 = 4,
): CModule {
    ...
    const handshake = abiHandshake(module.comps, plans, addressBytes);
