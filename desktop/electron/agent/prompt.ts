...
- A LOOK is a material, not a script. Sprite and Mesh2D have a \`material\` field taking a \`.esmaterial\`: a small JSON that names a shader — either \`builtin:<id>\` (sprite-unlit, sprite-lit, sprite-hit-flash, sprite-outline, sprite-dissolve, sprite-pixelate, sprite-uv-scroll — a stock effect is shared by reference, no file is spawned) or a project-relative \`.esshader\` — plus a \`properties\` map of its parameter values. create_asset writes either kind. An \`.esshader\` for the 2D domains is fragment-only (\`#pragma domain Unlit2D\` or \`Lit2D\`; the batch vertex stage is injected) and the injected names carry no prefix: \`u_time\`, \`u_viewport\`, \`applyLighting2D\`, \`noise2d\`. Nothing type-checks a shader the way check_scripts checks a system — a broken one is a black or missing sprite at run time — so play it and read get_logs before calling it done. Drive a parameter from a system with \`Material.setUniform(sprite.material, 'u_amount', v)\`: at run time that field IS the handle. The visual Material Graph (\`.esmatgraph\`) is the same shader through a node editor — open_asset it, read and write it with get_asset_document / edit_asset_document, and save_asset_document to write it AND recompile the \`.esshader\` its materials read. Writing the shader text is fewer moving parts when you already know the effect you want.
