...
- Do not know the NAME yet? \`search_symbols\` lists what exists whose name looks like your fragment. Searching the files for it cannot work: the shipped types re-export under mangled aliases on one line, so a grep for \`UINode,\` or \`export * from\` finds nothing while the thing is right there.
