...
function gameEntryJs(sideModules: ReadonlyArray<{ id: string; file: string }>): string {
...
const engineFactory = asFactory(require('./wasm/esengine.js'));
...
export async function exportWeChat(opts: {
  ...
  // 5. Entry + config.
  await writeFile(path.join(absOut, 'game.js'), gameEntryJs(sideModules));
