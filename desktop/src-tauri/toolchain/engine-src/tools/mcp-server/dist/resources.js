...
export function registerResources(server, bridge) {
    server.resource('editor://sdk-api', 'editor://sdk-api', { description: 'ESEngine SDK API reference: defineComponent, defineSystem, Query, Schedule, built-in components, script template' }, async () => ({
        contents: [{
                uri: 'editor://sdk-api',
    ...
    server.resource('editor://components', 'editor://components', { description: 'All registered component types grouped by category' }, async () => {
    ...
    server.resource('editor://assets', 'editor://assets', { description: 'All project assets with UUIDs, paths, and types' }, async () => {
    ...
    server.prompt('create-game-object', 'Guide: create a game entity with components and script', [], async () => ({
        messages: [{
                role: 'user',
                content: {
                    type: 'text',
    ...
    server.prompt('inspect-ui', 'Inspect UI: capture screenshot, get selection, entity data, and console logs', [], async () => ({
        messages: [{
                role: 'user',
                content: {
                    type: 'text',
    ...
    server.prompt('review-layout', 'Review layout: capture screenshot, get panel layout, and render stats', [], async () => ({
        messages: [{
                role: 'user',
                content: {
                    type: 'text',
