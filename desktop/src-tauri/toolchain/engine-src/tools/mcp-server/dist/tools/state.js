import { z } from 'zod';
export function registerStateTools(server, bridge) {
    server.tool('get_console_logs', 'Get recent console logs from the editor', {
    ...
    server.tool('get_panel_layout', 'Get the list of editor panels and their positions', {}, async () => {
        const result = await bridge.get('/state/layout');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('get_project_settings', 'Get project settings (all non-default values, or specific keys)', {
    ...
    server.tool('get_build_status', 'Get recent build history entries', {}, async () => {
        const result = await bridge.get('/state/build');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('get_render_stats', 'Get current frame timing and system performance data', {}, async () => {
        const result = await bridge.get('/state/render-stats');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('list_assets', 'List all assets in the project, optionally filtered by type', {
    ...
    server.tool('get_asset_info', 'Get detailed information about a specific asset by UUID or path', {
    ...
    server.tool('get_timeline_data', `Get timeline asset data. Returns { duration, wrapMode, tracks[] }.
Use list_assets with type="timeline" to find timeline UUIDs, or get_entity_data to read TimelinePlayer.timeline field.`, {
    ...
    server.tool('update_timeline_data', `Update a .estimeline asset file. Provide uuid/path plus any top-level fields to patch (tracks, duration, wrapMode).
...
5. Set loop playback: set_property({ entity: id, component: "TimelinePlayer", field: "wrapMode", value: "loop" })`, {
    ...
    server.tool('get_asset_meta', 'Get full asset meta: UUID, importer settings, labels, address, group, platform overrides', {
        uuid: z.string().optional().describe('Asset UUID'),
        path: z.string().optional().describe('Asset relative path'),
    }, async (args) => {
        const params = args.uuid ? `uuid=${args.uuid}` : `path=${encodeURIComponent(args.path)}`;
    ...
    server.tool('update_asset_meta', 'Update asset meta: labels, address, group, importer settings, platform overrides', {
    ...
    server.tool('ensure_asset_meta', 'Generate .meta file for an asset if it does not exist, returns the UUID', {
    ...
    server.tool('create_asset', 'Create a new asset file with default content and auto-generate .meta', {
    ...
    server.tool('delete_asset', 'Delete an asset file and its .meta file', {
        uuid: z.string().optional().describe('Asset UUID'),
        path: z.string().optional().describe('Asset relative path'),
    }, async (args) => {
    ...
    server.tool('rename_asset', 'Rename an asset file and update its .meta file', {
        uuid: z.string().optional().describe('Asset UUID'),
        path: z.string().optional().describe('Asset relative path'),
    ...
    server.tool('list_components', 'List all available component types grouped by category (builtin, ui, physics, script, tag)', {}, async () => {
        const result = await bridge.get('/components/list');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('get_component_schema', 'Get detailed schema for a component: fields, types, defaults, ranges, and constraints', {
    ...
    server.tool('list_menus', 'List all editor menu items with IDs, labels, shortcuts, and enabled state', {}, async () => {
        const result = await bridge.get('/menus/list');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('get_scene_metadata', 'Get current scene info: name, file path, dirty state, design resolution, undo/redo status', {}, async () => {
