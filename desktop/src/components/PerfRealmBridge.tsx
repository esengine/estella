...
export function PerfRealmBridge() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!alive) return;
      cache = s
        ? { phaseMs: s.phases, costs: s.costs, drawCalls: s.drawCalls, triangles: s.triangles, sprites: s.sprites, entities: s.entities, gpuMs: s.gpuMs, cppScopes: s.cppScopes, cppCounters: s.cppCounters, gpuScopes: s.gpuScopes, wasmBytes: s.wasmBytes, vramBytes: s.vramBytes }
        : null;
    };
    void poll();
