...
/**
 * @file  SettingsRow.tsx — one registered setting, rendered. The control picked
 *        by `type`, the label, the description, the live status line and the
 *        reset affordance.
 *
 *        Its own file because a setting is edited in more than one window: the
 *        settings dialog renders whole sections of them, and the Package Project
 *        dialog renders the rows a packaging target declares (`Setting.platform`)
...
/**
 * A credential: type it once; from then on the row can only say one is held, or
 * forget it. No reveal affordance, because main never hands the value back (see
 * settings/types.ts) and a row that could show it would have to keep it here. The
 * draft survives a refused store so a paste is never lost.
 */
 ...
 * the width of a table cell. Unset shows a labelled button, not a blank field: a
 * missing key is the likeliest reason a configured-looking row does nothing.
 */
...
export function Row({ setting }: { setting: Setting }) {
...
/**
 * A content group: a header + its rows. Conditional rows are filtered here rather
 * than inside {@link Row} — a row that renders nothing still runs its hooks, and a
 * group with every row hidden must take its header with it.
 */
export function Group({ label, settings }: { label: string; settings: Setting[] }) {
  const visible = settings.filter((s) => s.visibleWhen?.() !== false);
...
export function groupByGroup(settings: Setting[]): { label: string; settings: Setting[] }[] {
  const order: string[] = [];
