...
export const projectMessages = defineMessages({
    ...
    'proj.applyStructuralTitle': { en: 'Apply structural changes to prefab?', zh: '将结构改动应用到预制体？' },
    'proj.applyStructuralBody': { en: 'This rewrites "{name}" for all of its instances: {overrides} override(s), {added} added, {removed} removed.', zh: '这会重写"{name}"及其所有实例：{overrides} 项覆盖，新增 {added}，移除 {removed}。' },
