...
/** Where the eye sits relative to the character: behind it and above it. */
const OFFSET = { x: 0, y: 380, z: 900 };
...
export const cameraFollowSystem = defineSystem(
    ...
    (cameras, characters, time) => {
        let target: { x: number; y: number; z: number } | null = null;
        for (const [, transform] of characters) {
