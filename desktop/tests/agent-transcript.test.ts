...
const ended = (over: Partial<{ steps: number; mark: unknown; reason: 'end_turn' | 'aborted' | 'error' | 'refusal' }> = {}): AgentEvent =>
  ({ type: 'turn_end', steps: 0, mark: null, endMark: null, tx: null, files: [], acceptance: { verdict: 'unverified', results: [] }, reason: 'end_turn', ...over });
...
describe('which entities a turn touched', () => {
  ...
  const turn = (entries: AgentToolEntry[], id = 0): AgentTurn => ({
    ...
    steps: 1, mark: { seq: 1 }, endMark: null, tx: null, files: [], acceptance: { verdict: 'unverified', results: [] }, reason: 'end_turn', startedAt: 0, endedAt: 1,
  });
