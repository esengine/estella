...
describe('exportGame (wechat) — open data context', () => {
  ...
  it('a package can be the context, through the one line the project writes', async () => {
    // Which is how a friends board ships without the engine carrying one: the
    // project owns the file, a dependency owns the code, and the bundler
    // resolves it like any other import.
    const { root: r, out: o } = scaffold("import 'estella-plugin-fake/open-data';\n");
    const pkg = path.join(r, 'node_modules', 'estella-plugin-fake');
    mkdirSync(pkg, { recursive: true });
    writeFileSync(path.join(pkg, 'package.json'), JSON.stringify({
      name: 'estella-plugin-fake', version: '1.0.0', exports: { './open-data': './od.js' },
    }));
    writeFileSync(path.join(pkg, 'od.js'), 'globalThis.__PACKAGE_BOARD__ = 1;\n');

    const res = await exportIt(r, o);
    expect(res.ok).toBe(true);
    expect(readFileSync(path.join(o, 'open-data', 'index.js'), 'utf8')).toContain('__PACKAGE_BOARD__');
    expect(JSON.parse(readFileSync(path.join(o, 'game.json'), 'utf8')).openDataContext).toBe('open-data');
  }, 60_000);
