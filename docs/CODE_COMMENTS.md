...
- **不要在注释里写内部/临时的流程代号**（审计编号如 `A20`、工单号、章节号如 `§4.2`、私有缩写）——代码读者查不到、而且会过时。
- **不要引用读者打不开的文档。** 指向文档时那份文档必须是**仓库里已提交的文件**（`docs/CODE_COMMENTS.md`、`README.md` 这类）；`docs/REARCH_*.md` 全都在 `.gitignore` 里，指向它们等于指向别人机器上的文件。注释要**自己把话说完**：把章节里那句判断直接写下来，而不是让读者去取一份他没有的文档。`file.ts:123` 这种可定位的引用可以。由 `tools/check-comment-style.mjs` 的 `unreachable-doc` 规则按 git 索引判，只看新增行。
...
- [ ] 内联注释解释的是"为什么"，没有复述代码？
- [ ] 没有注释掉的死代码、没有裸 TODO？
- [ ] 注释**自包含**，没有内部流程代号（如 `audit A20`、工单号、`§4.2`）？指向文档时那份文档确实已提交？
