...
**剩余**：Phase 4 用户手写 `.esshader` 的 cook 期 naga 转换（归 RC6 资产管线；实测边界——mat-lit 等场景资产 shader 无孪生，WebGPU 下按设计描述性跳过）；WebGPU 异步 readback seam（`readPixels` stub，材质预览断言跳过）；CI 无 WebGPU 适配器（webgpu verify 仅本地）。
