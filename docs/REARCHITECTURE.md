...
### RC11 网络复制 — ✅ N0–N5 已落地（2026-07-08）；余项为编辑器联调工作流与需求驱动的后置
...
- **余项**：编辑器多 PlayRealm 本地联调工作流（`PlayRealm.ts` 单例改实例 + utilityProcess 起本地服务器，审计确认无结构障碍——独立的编辑器 UX 工作项）；headless 资产模式（服务器加载含纹理场景需"跳过纹理/音频驻留"通道，现为 fail-loud）；ghost 物理/AI 让位门控语义（RigidBody ghost 是否强制 kinematic）待真实 demo 驱动；本端预测+和解、relevancy/兴趣管理、C++ 批量快照优化、带宽量化——全部需求驱动不抢跑。
