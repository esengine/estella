...
**剩余**：glslang 侧 vendoring **待用户拍板**——加 KhronosGroup/glslang submodule 被权限门拒（引入外部代码源需明示同意）;官方 emscripten/JS 目标（ENABLE_GLSLANG_JS）可产 wasm,或改用 Khronos release 二进制下载方案;当前 glslangValidator 仍需 PATH 提供（Vulkan SDK 附带）。wechat/playable 导出线采用 CA 时复用 sourcePath 机制；CI 无 WebGPU 适配器（webgpu verify 仅本地）。
