...
**剩余**：CA 构建的运行时路径解析（gameHost 用 sourcePath 喂 Catalog buildPath + resolvePlayAssetRef 路径分支;**注意存量缺口**——材质内相对引用按 staged 目录解析,在 CA 构建下对 @uuid: 引用同样破损,需 loader 侧以逻辑路径做相对解析,牵 Assets 内部,单独切片）；转换器 vendored wasm 交付（glslang wasm + naga wasm32-wasi,仿 build-tools/basis）；CI 无 WebGPU 适配器（webgpu verify 仅本地）。
