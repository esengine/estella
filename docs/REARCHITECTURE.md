...
- **函数绑定面握手（2026-07-08）**：病灶所述"`ESEngineModule` 接口是手工镜像、与 `bindings/*.cpp` 的真实导出无任何交叉校验"的收口第一步——新增 `sdk/tests/binding-surface.test.ts`（`cpp-contract.test.ts` 同款测试期握手，随 SDK vitest 进 push 门禁）：解析 `WebSDKEntry.cpp`/`TilemapBindings.cpp` 的全部 `emscripten::function` 注册与 `wasm.ts` `ESEngineModule` / `tilemapAPI.ts` `TilemapModule` 声明面，**双向存在性断言**（幽灵声明与未镜像绑定都 RED），另对 embind class 面（`ResourceManager`/`EstellaContext`）逐方法双向对齐。首跑即抓到 16 处存量漂移并已全部对齐：幽灵 `getSpineBounds`+`SpineBounds`（spine 迁侧模块后 C++ 函数仍在但从未注册，TS 声明悬空）、未镜像的 `renderer_setDeltaTime` 与 12 个 `tiled_getObject*`、`CppResourceManager` 缺 3 个 refcount getter。同日三个后续切片全部落地：① **侧模块入守卫**（`346065a9`）——physics（4 文件 96 个 `EMSCRIPTEN_KEEPALIVE` C 导出 vs `PhysicsWasmModule`）、spine（原始 `_spine_*` 成员 ∪ `cw('name',…)` cwrap 表 vs 40 个 KEEPALIVE，抓到未镜像的 `spine_setClippingEnabled`）、basis（TS vs CMake `EXPORTED_FUNCTIONS`，链接期已对 C++ 校验，补上 TS↔CMake 这条边）；② **死绑定退役**（`bc4bc423`，wasm 重建 + sprite 像素 verify PASS）——`renderer_setDeltaTime` 整条通道（注册/定义/`EngineState.delta_time` 字段写后无人读/TS 声明）与 `RendererBindings.hpp` 的旧 spine_* 声明家族（`SpineBounds`+9 个无定义无调用者的裸声明）删除；③ **签名元数握手**——逐函数钉参数个数（TS 类型运行时擦除，元数漂移的声明会让所有调用点编译通过、只在路径被执行时爆炸；现在测试期 RED），覆盖主模块注册（含 lambda/`select_overload`）、`ResourceManager` class 面（embind 丢实例参，TS 元数 = C++ 元数−1）、physics/spine（raw + cwrap argTypes 长度）/basis，另有解析器自检用例防"空转假绿"。**拍板：本轨道以握手收口，不做 EHT 生成**——存在性 + 元数已双向钉死、embind/cwrap 在调用期校验值类别，生成反而把手写接口的富类型（结构化返回值、JSDoc）退化为贫类型（与"不生成 defineBuiltin"同构决策）；已知接受的残余 = TS 形参名与 C++ 不一致这类无自动化可查的语义漂移。**至此根因 1-5 的全部病灶字面收口。**
...
**cook 集成第一层（2026-07-08）——资产依赖图现代化**：assetDb 依赖图从"仅 scene/prefab 的 @uuid:"扩展为**任意 JSON 资产 × 三种引用形态**——`@uuid:`、裸 uuid（anim 帧的 extractUuid 形态，仅命中索引才建边，防实体 id 误伤）、**路径引用**（项目路径 + 相对本文档目录,只有真实资产命中才建边零误报）；扫描类型 scene/prefab/material/tileset/animclip/statemachine/behaviortree。真实项目（路径引用内容）的材质→shader→纹理链自此进入 cook 闭包（此前 deps 全空,整链被静默剔除）。cook manifest 新增 `sourcePath`（逻辑身份,内容寻址改名后路径引用的运行时映射依据）；`.esshader` 无 WGSL 孪生 → cook 告警（生成是开发期步骤,保 cook 确定性 + 零转换器依赖）。
**cook 集成第二层（2026-07-08）——CA 构建运行时路径解析闭环**：三件套——① cook 期把材质的**相对引用重写为逻辑项目路径**（内容寻址摧毁了相对解析依赖的目录结构;非 assets/-根的逻辑路径写成 `/<logical>`,双拼写皆注册;@uuid:/URL/非资产字符串原样通过）,顺带根治"@uuid: 引用的材质在 CA 构建下内部相对引用同样破损"的存量缺口——loader 零改动;② `resolvePlayAssetRef` 增 pathMap 参数（逻辑→staged,**先于扩展名嗅探**,.png→.ktx2 换名对 KTX2 检测可见,与 uuid 分支同契约）;③ 生产首次喂 `Catalog` buildPath（`AssetsOptions.catalog` 一直存在但空转——loader 内部文本 fetch 如材质取 shader 走 `getBuildPath`）,经 `RuntimeInitConfig.catalog`/`PlayRealmRuntimeConfig.catalogData` 管道在 per-App Assets 首次创建时注入,gameHost 从 manifest sourcePath 构建两张映射。验证:cooked e2e 升级为双象限——绿 KTX2 精灵（uuid 引用）+ **红材质精灵（全路径引用链:场景→材质项目路径→shader/纹理相对引用）**,发行运行时逐像素 PASS（[0,181,0]/[255,0,0]）。
**音频并入单一解析通道（2026-07-08）**：Audio 的自有 fetch 曾绕过 resolveRef（`baseUrl` 前缀并行逻辑,CA 构建下 `playSFX('assets/…')` 404）——新增 `Audio.setRefResolver`,play realm 统一接 `resolvePlayAssetRef`（编辑器 realm 输出与旧 baseUrl 前缀逐字符一致零行为变化;cooked 构建命中 logical→staged 映射;uuid 音频引用顺带可用）。`baseUrl` 保留为未接线宿主的 legacy 前缀。
**naga vendored（2026-07-08）**：SPIR-V→WGSL 侧零外部依赖——`tools/naga-spv2wgsl`（30 行 stdin→stdout WASI 封装 crate,Cargo.lock 钉版）编 wasm32-wasip1 产物入库 `build-tools/shader-twins/naga-spv2wgsl.wasm`（674KB,basis 先例同款 gitignore 白名单）,`naga.mjs` 经 node:wasi 进程内运行;gen-shader-twins 改走 vendored 路径,**全部 fixture 重生成零 diff**（与 naga-cli 30 逐字节一致）。
**glslang vendored（2026-07-08,用户批准 submodule）**：GLSL→SPIR-V 侧也零外部依赖——`third_party/glslang` submodule（钉 16.3.0）+ `tools/glslang-wasm/wrapper.cpp`（薄 C ABI,精确镜像 `glslangValidator -V --auto-map-locations`:Vulkan 1.0 语义、auto-map locations、无优化器）经 emscripten 构建入库 `build-tools/shader-twins/glslang-compile.{mjs,wasm}`（1.8MB）。**gen-shader-twins 自此零 PATH 依赖**（只需 build:web 产物）;全 fixtures 经全 vendored 管线重生成**零 diff**（与本机 glslangValidator 16.0.0 + naga-cli 30 逐字节一致——跨 glslang 小版本仍稳定）。CI 新增两守卫（verify-render job）:孪生覆盖检查（--check 全 fixtures）+ **管线冒烟**（剥孪生临时副本走完整 GLSL→SPIR-V→WGSL,每次 CI 真跑两个 wasm 转换器）。坑:工具改 `process.exitCode` 优雅退出——硬 process.exit 与存活的 emscripten 运行时在 Windows 上有拆除竞态（STATUS_STACK_BUFFER_OVERRUN）。
**wechat/playable 并入 sourcePath 解析（2026-07-08,三线归一收官）**：wechat——AddressableManifest 的 `address` 字段承载逻辑源路径（ManifestModel 索引 address 双拼写入 byKey → resolvePath 直接命中;initWeChatRuntime 从 address 构建 Catalog buildPath 喂 initRuntime,与 web 宿主同款双映射）,exportWeChat 在 staging 改名时发射 address,exportGame 顺带补上 wechat 分支漏转发 contentAddressed/compressTextures 的穿线缺口。playable——导出器发射紧凑 `__GAME_PATHMAP__`（逻辑路径→`@uuid:` 键）,运行时**内存内别名**到内嵌 assets 映射（别名共享 data-URL 字符串,载荷零重复;fetch/图像解码/音频三通道读同一张表）。三条导出线自此共用同一套"逻辑身份 → staged 物理"解析语义。
**WebGPU 验证入 push 门禁（2026-07-08,f236fc73,最后一个"环境性限制"证伪）**："CI 无 WebGPU 适配器"是约定不是事实——Dawn 经 Electron 自带的 SwiftShader Vulkan（`--use-webgpu-adapter=swiftshader`）在无 GPU runner 上获得真实适配器;本地强制软件适配器与硬件运行逐像素一致（grid diff 22098 相同）。headless-verify 增 `enable-unsafe-webgpu` + `ESTELLA_VERIFY_WEBGPU_ADAPTER` 开关;build.yml verify-render 新增 WebGPU 段,四场景与 GL battery 同断言（sprite+grid / parallax-shape / mat-lit-point 生成孪生+Lit2D / mat-tint+preview 异步 readback）,分支预演全绿后 fast-forward。**双后端像素验证自此每次 push 自动跑。**
...
### RC11 网络复制 — 🟡 N0–N4 已落地（2026-07-08）；剩 N5 宿主故事
...
- **N3 快照插值**：delta 帧落进 per-field 时间序列（稀疏 delta 无需重建全快照）；渲染时钟落后最新服务器 tick 一个可配延迟（默认 2），PostUpdate 逐帧采样——f32 标量/向量叶 lerp、四元数就近 cover nlerp、离散值 hold 到样本时刻；时钟按渲染时间推进 + 向目标软纠偏（burst 抖动弯时间而非瞬移），钳在最新数据（不外推）故静默后精确收敛到终值。`interpolationDelayTicks:0` = N2 立即语义（N2 闸门测试钉住）。
...
- **待续（N5 宿主故事）**：node 平台适配器（fetch/readFile/instantiateWasm 薄层 + `createSocket` 的 ws 侧）+ `createHeadlessApp`（createWebApp 减渲染装配）+ 真 WebSocket e2e（node 权威服务器 + headless 客户端；服务器侧 ws 库选型待拍板）+ 编辑器多 PlayRealm 联调（`PlayRealm.ts` 单例改实例，审计确认无结构障碍）。另有已识别的后置项：ghost 物理/AI 让位门控语义（RigidBody ghost 是否强制 kinematic）待真实 demo 驱动拍板；`#pragma switch` 用户 shader 手写孪生残余（WGSL 战役遗留，与本战役无关）。
