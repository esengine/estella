...
**剩余**：转换器 vendored wasm 交付（glslang wasm + naga wasm32-wasi,仿 build-tools/basis）；音频路径引用在 CA 构建下仍走 assetBaseUrl 裸 fetch（Audio 有独立 loader,未过 resolveRef——已知缺口）；wechat/playable 导出线采用 CA 时复用同一 sourcePath 机制；CI 无 WebGPU 适配器（webgpu verify 仅本地）。
