...
- **L1（数据模型持有）**:`ProjectStore` load 后**保留** SceneData(连同未知组件/字段/visible/textureMetadata),不再投影即弃;加稳定 editor id。验证:load 一个含未知组件的场景,数据模型完整(未知组件在)。
- **L2（增量投影层）**:数据模型 → world 的投影 + 增量 patch(改一个字段/增删实体 → 局部更新 world 预览,不全量 resetWorldTo)。验证:编辑数据模型 → 视口即时反映、不闪、play 状态不丢、id 不漂。
- **L3（命令改道）**:`SceneCommands` 写数据模型(承接 E2 边界)+ 触发 L2 投影;`SceneQuery`/inspector 读数据模型。验证:所有编辑路径行为不变(回归)+ 未知组件/字段在编辑期始终保留。
- **L4（无损保存）**:`save` = 序列化数据模型(非 `serializeScene(world)`);删 `lossy` 标志 + 拒绝逻辑 + `restoreAssetRefs`。验证:**golden 往返**——load→save **逐字段一致**(含未知组件、未知字段、visible、asset `@uuid:`、textureMetadata、double 精度)。
- **L5（play 快照取数据模型）**:`EngineHost.setRunMode` 快照数据模型而非 `serializeScene(world)`;Stop 恢复数据模型。验证:play→编辑大量改动→stop,场景逐字段还原;接 play==ship。
