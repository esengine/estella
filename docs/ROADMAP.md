...
**完成的样子。** 给 `frame-bench` 加一个 bystander 维度并进 `--gate`：同一台机器、同一个
编译构建，5,000 个 mover 在小世界和大世界里的每实体代价比值回到 ~1.0x。不加这一列，这个
回归和它当初一样是**静默**的 —— 它不报错、不改像素，只是慢。
