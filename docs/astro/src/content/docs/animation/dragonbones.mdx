...
<Aside type="note">
  `color` **multiplies** each slot's authored colour rather than replacing it, so
  what an artist set in DragonBones Pro survives being tinted — and opaque white
  (the default) is the tint that changes nothing, which is why clearing the field
  restores the original instead of leaving the last colour stuck.
</Aside>
...
| `setColor(entity, r, g, b, a)` | Tint the whole armature (0..1), multiplied onto slot colours. |
| `setEnabled(entity, on)` | Remove from the frame entirely (freeze **and** hide). |
| `setEntityProps(entity, props)` | Set `{ skeletonScale?, flipX?, flipY?, layer?, playing?, timeScale?, color? }` at once. |
...
`setEnabled` and the component's own `enabled` field drive the same switch: writing
the field (in the inspector, or via `world.set`) wins at the moment it is written,
and `setEnabled` holds between such writes. The editor's Outliner eye rides the
same channel, which is why hiding a skeletal entity hides it in the viewport
without touching the `enabled` you authored.
...
- [Spine Animation](/docs/animation/spine/) — the other skeletal runtime, and how it differs.
- [Animation](/docs/animation/overview/) — sprite flipbooks and the Animator state machine.
- [Assets](/docs/assets/overview/) — how skeleton and atlas assets are imported and referenced.
- [Build & Export](/docs/publishing/overview/) — what a package contains per target.
