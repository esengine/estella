...
you'll normally write. The path is relative to the **project root**, not to the
assets folder, so it starts with `assets/` — the spelling the Content Browser
shows and the one the cook's manifest keys on:

```typescript
const tex = await assets.loadTexture('assets/textures/player.png');
```

A ref that drops the root (`'textures/player.png'`) resolves against the project
root all the same, finds nothing there, and fails the load with a 404.
...
A `.json` that is not a Spine skeleton or a DragonBones pair is a **data asset** — the
game's own tables, tuning and dialogue. It loads like anything else:

```typescript
const { data } = await assets.loadJson<LevelTable>('assets/data/levels.json');
```

`T` is what you expect the file to hold; nothing validates it, so this is `JSON.parse`'s
guarantee and not a schema's.

Loading it as an asset rather than fetching the path yourself is not ceremony. A data
asset has a uuid, so moving the file does not break what points at it; it is parsed once
and reference-counted; it can live in a lazy [group](#addressable-groups-lazy-loading) or
come from a CDN; and [hot update](/docs/publishing/hot-update/) can replace it in a shipped
game. A hand-rolled `fetch` of a project path gets none of that — and it *works in the
editor*, which serves the whole project, while the build ships only assets.

Data assets ship whether or not anything references them — see
[Shipping what only code names](#shipping-what-only-code-names), where they are one of
the two built-in exceptions.

The project's own configuration is not data: `package.json`, `tsconfig.json` and their
kind never become assets.
...
This grouping is what maps onto [WeChat subpackages](/docs/publishing/wechat/) for
on-demand download.
...
- **Compress textures** — encode PNGs to GPU-ready **KTX2** (Basis Universal),
  ...
  decide this, with a tab per platform (below), so a texture can be compressed
  harder for a phone than for the web; the Package dialog's **Compression** switch
  only chooses whether to honor those settings or skip them all.
...
![The Inspector's Platform Overrides strip — a tab per platform, Android selected with Max Size, Compress and Format](@/assets/guides/en/import-platform-overrides.png)

*A texture's **Platform Overrides**: each target gets its own tab, and a platform that overrides nothing follows the Default settings above it.*
...
<Aside type="tip">
  ...
  Two types never need it, because nothing can ever reference them: **locale
  tables** (`Text` carries keys, not paths) and **data assets** (`.json`, named by
  the code that loads them). Both always ship.
</Aside>
...
// stats.retainedBytes         — CPU copies kept to survive a GPU loss (below)
...
### Memory kept for a lost GPU

When the graphics context is lost (a driver reset, a mobile browser reclaiming
the GPU), the engine rebuilds every texture and buffer behind the handles your
game holds. Most content comes back from where it came from — an asset is
loaded again, a text page is repainted from its glyphs, a skeleton's atlas page
is decoded again. What has no such source (a mesh built at runtime, a tile
collection composed at load) is kept as a CPU copy, which counts against your
memory but not against any GPU figure. `stats.retainedBytes` is that copy; the
engine's own is about 28 KB. It warns once when the copy grows past
`RuntimeConfig.retainedBudget` (**16 MB** by default, `retainedBudget` in the
build config, or `setRetainedBudget(bytes)`; 0 never warns).

What the engine cannot put back is content **your game drew**: a render texture
is GPU storage, so a rebuilt device hands it back blank. `onDeviceRestored` says
when to draw it again, and `RenderTexture.contentLost` says which target needs
it — a target a camera renders every frame never does.

```typescript
import { onDeviceRestored, RenderTexture } from 'esengine';

const minimap = RenderTexture.create({ width: 256, height: 256 });
onDeviceRestored(() => {
  if (!RenderTexture.contentLost(minimap)) return;
  drawMinimap(minimap);                  // your own draw
  RenderTexture.contentDrawn(minimap);   // …and say it holds pixels again
});
```

For watching these numbers frame-over-frame in the editor and at runtime, see
[Profiling](/docs/performance/profiling/). Per-texture **sampling/filtering**
parameters (pixel-art nearest vs. smooth linear) are covered in the
[Sprites guide](/docs/graphics/sprites/).
...
- [WeChat MiniGame](/docs/publishing/wechat/) — how addressable groups map to subpackages.
- [Scenes](/docs/world/scenes/) — scenes reference assets and are cooked as entry points.
- [Materials & Shaders](/docs/graphics/materials/) — `loadMaterial` and shader handles.
