...
The `Sprite` component now supports `tileSize` and `tileSpacing` for seamless or spaced tiling:

- **Seamless tiling** (`tileSpacing = 0`): uses `uvScale` override with `GL_REPEAT` for zero extra draw-call overhead
- **Spaced tiling**: emits N×M quads with configurable gaps, auto-merged into a single draw call via the batch system
- Both modes respect sprite flip and pivot settings
