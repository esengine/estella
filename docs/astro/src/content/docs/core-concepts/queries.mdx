...
<Aside type="note">
When using `Mut` queries, the engine validates that each entity still exists before writing back mutations. This means it is safe to despawn entities via `Commands` during iteration — the write-back for a despawned entity is silently skipped rather than crashing.
</Aside>
