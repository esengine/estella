...
- **Pan** — drag with the middle mouse button (or hold **Space** and drag, for a trackpad).
- **Turn** — drag with the **right** mouse button once the view is a spatial one
  (perspective, or already turned), or **Alt-drag** from any view. A drag grabs the
  scene: the world turns the way the pointer moves. On a flat 2D view right-drag
  pans, as it always did — there is nothing to walk around.
...
render — Cameras, `Light`, colliders, joints, particle emitters — are drawn as
