...
```typescript
import { NavGrid, findPath, pathToWorld } from 'esengine/ai';
...
```typescript
import { navGridFromTiles } from 'esengine/tilemap';
...
- **`quaternionToAngle2D(z, w)`** — 2D facing angle in radians from a `Transform`
...
```typescript
import { quaternionToAngle2D, Transform, registerCondition } from 'esengine';
import { senseTarget } from 'esengine/ai';
...
registerCondition('seesShrine', (ctx) => {
  ...
  const facing = quaternionToAngle2D(tf.rotation.z, tf.rotation.w);
  return senseTarget(
...
```typescript
import { perceptionPlugin, fsmPlugin, btPlugin, navPlugin } from 'esengine/ai';
