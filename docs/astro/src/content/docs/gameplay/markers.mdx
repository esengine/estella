...
`type`. Add a static [`RigidBody`](/docs/guides/physics/) and a sensor collider and
...
- It's **`Query`-able** — one code path reads markers you placed by hand *and*
  markers imported from a [Tiled](/docs/guides/tilemap/) `.tmj` object layer.
...
one. A **Trigger Area** carries a sensor collider, so it needs [physics](/docs/guides/physics/):
...
[collider gizmo](/docs/guides/physics/) as any other body.
...
notice them on dedicated [collision layers](/docs/guides/physics/#collision-filtering),
so each sensor's mask narrows what it considers. See
[Collision & sensor events](/docs/guides/physics/#collision--sensor-events) for the
full event surface.

## Imported from Tiled

When a [`Tilemap`](/docs/guides/tilemap/) loads a `.tmj` map, its **object layers**
...
- **Trigger Areas are sensors** — for anything you want to *block* (a wall, a ledge),
  paint a [collision layer](/docs/guides/tilemap/#collision-obstacle-layers) or use a
  non-sensor collider instead.

## See also

- [Tilemaps](/docs/guides/tilemap/) — collision (obstacle) layers and Tiled object
  layers that feed the same Markers.
- [Physics](/docs/guides/physics/) — sensors, collision filtering, and the
  `PhysicsEvents` resource behind Trigger Areas.
- [Scenes](/docs/guides/scene/) — Markers and Trigger Areas serialize with the scene.
- [Event Binding](/docs/guides/events/) — run an action when something enters a Trigger
