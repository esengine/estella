...
description: Draw beyond components — immediate-mode primitives with Draw, vector paths with Graphics, custom triangle meshes with MeshRenderer and the Geometry API, and offscreen render textures.
---
...
| `ShapeRenderer` / `MeshRenderer` | **Components** — entities with sorting layers, materials, editor gizmos. | Shapes/meshes that are part of the scene (see [Sprites](/docs/graphics/sprites/)). |
...
`MeshRenderer` is a component that renders an arbitrary indexed triangle list — 
...
Where `MeshRenderer` is the *component* path, `Geometry` is the *immediate* one: it
