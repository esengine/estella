...
- **WebGPU, more than one casting light.** On the WebGPU backend a scene with two or
  more shadow-casting lights loses the shadows of some of them; the lights still light
  it. One casting light is correct on both backends, and WebGL2 is correct throughout.
  The criterion that shows it (`shadow2d-light-cap`) runs on WebGL2 only, and says so.
