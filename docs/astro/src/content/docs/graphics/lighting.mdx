...
:::caution
Estella has no lightmapper yet — nothing in the editor *produces* an atlas. The reading
side is what shipped here, so a bake authored elsewhere (Blender's Cycles, a Substance
bake, any renderer that writes a lightmap and its UV layout) can be brought in and read
today. Baking from inside the editor is the next piece.
:::
