...
<Aside>
  Lights are still placed in the plane: a `Light2D` has no third coordinate, so
  distance is measured in xy and a point light's height comes from its radius. The
  normal and the view vector are the three-dimensional part of the model.
</Aside>
