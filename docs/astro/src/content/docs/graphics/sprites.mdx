...
description: Draw 2D images with the Sprite component — textures, tint and transparency, size and pivot, flipping, sorting layers and draw order, y-sort and depth layers (2.5D), parallax — plus texture filtering, hit-testing, cache-as-bitmap, ShapeRenderer primitives, sprite filters, motion trails, and world-space bitmap text.
---
...
**`ShapeRenderer`**. Everything is drawn through a [Camera](/docs/graphics/camera/), and
draw order is controlled by **sorting layers**.

## Your first sprite

A sprite needs a `Transform` (where it is) and a `Sprite` (what it draws). Load a
texture through the [`Assets`](/docs/assets/overview/) resource and put its handle on the
sprite:
...
| `material` | `0` | A custom [material](/docs/graphics/materials/) handle (`0` = the default sprite shader). |
...
[Sequencer](/docs/animation/timeline/) for flashes and fades.
...
![The same sprite rotated about three different pivots — the pivot dot stays fixed while the quad swings around it: center spins in place, a corner swings out, bottom-center pins the feet](@/assets/guides/sprite-pivot.svg)
...
Storing a fraction is what lets a pivot survive a resize, but it isn't how anyone
thinks about one — see [In the editor](#in-the-editor) for entering it in pixels,
picking it from the nine common positions, or dragging it on the sprite itself.
...
<Aside type="note">
  ...
  **physics meter scale**: how many design pixels make one Box2D meter (see
  [Physics](/docs/gameplay/physics/)). A sprite's displayed size is `size` ×
...
<Aside type="tip">
  ...
  fine ordering *within* a layer, state `sprite.order` — see below.
</Aside>

### Order within a layer

`sprite.order` is where a sprite sits **inside** its sorting layer: higher draws on
...
On a Y-sorted layer this is the *only* override there is, because z never reaches
that key at all. Without it, keeping a shadow under its owner meant moving the sprite
in Y and lying about where it stands.

### Sorting groups

An order is a promise about one sprite against **everything else in the layer**, and
that stops scaling as soon as two assembled characters overlap: raising a weapon above
the other character raises it above its own body too.
...
Two characters are then ordered against each other by their groups alone, and neither
one's parts can interleave with the other's.
...
<Aside type="caution">
  A group's members and an ungrouped sprite that happens to share the group's layer
  *and* order interleave in an unspecified way — "nothing lands between the members"
  is a promise about the group's own place, so give the group a place of its own.
</Aside>

### Sprite masks

A `SpriteMask` turns an entity's **own sprite** into a stencil: sprites drawn after it
...
Masking is **opt-in per sprite** (`Sprite.maskInteraction`), so adding a mask to a scene
...
<Aside type="caution">
  Where two masks overlap, the **nearer** one decides — it wrote last, and that is what
  the stencil holds. A sprite cut by the farther mask would disappear exactly inside the
  nearer one, so only one mask cuts any given sprite.
</Aside>
...
Same layer, same z? Then **scene order** decides: the engine draws entities in the
order the scene lists them, so the one further **down** the Outliner lands on top.
Dragging a row in the Outliner therefore changes what covers what — and the viewport
updates as you drop it, exactly as the running game will draw it. A dragged parent
takes its children along, so a child always keeps its position relative to its parent.

At runtime the same order is available without touching layers — useful for "bring
this card to the front":
...
Check a layer under **Project Settings → Rendering → Depth-sorted layers** and it
...
Which sprites take part is decided by their material's **blend mode**, because that
is what says whether a draw reads what is already on screen:
...
That split is not a setting — a translucent sprite that wrote depth would clip the
sprites behind it into hard black edges. So a sprite you want to occlude properly
needs an opaque material; leave the blend mode alone and it behaves as it always has,
just with things in front of it hiding it.

To look at any of this while authoring, switch the viewport's **2D / 3D** button
(scene toolbar) — the editor's own eye becomes a perspective one. It changes only
your view: the Game view always shows the scene's camera. Picking and dragging follow,
so a sprite at z = -400 is grabbable where it is drawn.

<Aside type="note">
  A layer is either y-sorted or depth-sorted, never both — y-sort *is* a depth
  projected from world Y, and a layer that declares both keeps y-sorting. Usable z
  range is ±`far` under an orthographic camera (±1000 by default) and `near`..`far`
  under a perspective one; content outside it is clipped away entirely.
</Aside>
...
- **Atlases**: show one cell of a packed sheet by setting `uvOffset` and `uvScale` to
  ...
  [Animator](/docs/animation/overview/) instead of by hand.)
...
dissolve, a distortion — see [Materials & Shaders](/docs/graphics/materials/).
...
see [2D Lighting & Shadows](/docs/graphics/lighting/).
...
widget instead — see [UI](/docs/ui/overview/).
...
resource does the first (see [Camera](/docs/graphics/camera/)); for the second the
...
picking for you (see [UI](/docs/ui/overview/)).
...
`Sprite.texture`. Under the hood this is a [RenderTexture](/docs/graphics/drawing/)
...
- Every field above is editable in **Details**; sprites, cameras, and lights show as
  gizmos in the viewport. See [The Editor](/docs/editor/overview/).
- **`pivot` is authored three ways**, all writing the same stored fraction:
  ...
  - The **`frac` / `px`** switch on the row enters it in pixels instead — `px` shows
    `pivot × size`, and typing `32` stores `32 / size`. The unit is a view; the scene
    always holds the fraction.
  - With the **pointer tool**, a single selected sprite gets a **pivot dot** in the
    viewport. Dragging it moves the pivot through the artwork *without moving the
    artwork* — the transform takes up the slack — so you can put the turning point on
    a foot or a hinge by eye. It hides under Move/Rotate/Scale, whose centre grab sits
    in the same place.

## See also

- [Textures](/docs/assets/textures/) — the image itself: formats, import settings, and how to read its width and height.
- [Animation](/docs/animation/overview/) — sprite-sheet flipbooks and tweened properties.
- [Custom Drawing](/docs/graphics/drawing/) — meshes and immediate-mode shapes beyond sprites.
- [2D Lighting & Shadows](/docs/graphics/lighting/) — normal maps and per-sprite lighting.
