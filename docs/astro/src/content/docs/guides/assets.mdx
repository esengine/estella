...
| Method | Returns | Description |
|--------|---------|-------------|
| `fetchJson<T>(path)` | `Promise<T>` | Load and parse a JSON file |
| `fetchText(path)` | `Promise<string>` | Load a text file |
| `fetchBinary(path)` | `Promise<ArrayBuffer>` | Load a binary file |

```typescript
const config = await assets.fetchJson<GameConfig>('assets/config.json');
const csv = await assets.fetchText('assets/levels.csv');
const data = await assets.fetchBinary('assets/tilemap.bin');
```
...
```typescript
const manifest = await assets.fetchJson('assets/addressable-manifest.json');
...
Typed load methods (`loadTexture`, `loadSpine`, `loadMaterial`, `loadPrefab`) cache by path. Loading the same path twice returns the cached result without a network request. Generic fetch methods (`fetchJson`, `fetchText`, `fetchBinary`) do not cache — each call fetches from the backend.
...
```typescript
const sceneData = await assets.fetchJson<SceneData>('assets/scenes/main.esscene');
...
For Playable Ad builds, the editor's build pipeline automatically embeds all referenced assets as data URIs into the output bundle. At runtime, `assets.loadTexture()` / `assets.fetchJson()` and other load calls resolve from the embedded data instead of fetching over the network — no code changes are needed.
...
Plugins can register custom asset type handlers so the scene loader knows how to discover and load your asset types:

```typescript
import { registerAssetHandler } from 'esengine';
import type { AssetFieldHandler } from 'esengine';

const myHandler: AssetFieldHandler = {
    async load(paths, assetServer, baseUrl, texturePathToUrl) {
        const results = new Map<string, number>();
        for (const path of paths) {
            // load your asset, return a handle
            const handle = await loadMyAsset(assetServer, path, baseUrl);
...
registerAssetHandler('my-asset-type', myHandler);
```

Built-in handlers (texture, material, font, spine, audio, tilemap, timeline, anim-clip) are registered automatically. Custom handlers are typically registered in a plugin's `build()` method.
