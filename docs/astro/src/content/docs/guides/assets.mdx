...
- **Compress textures** — encode PNGs to GPU-ready **KTX2** (Basis Universal),
  ...
  → S3TC, falling back to RGBA8). Toggle **Compress textures** in the Package dialog.
...
- **Auto-atlas** — put loose PNGs in a `<name>.atlas/` folder and the cook packs
  ...
  small sprites ship as one texture and batch in one draw call. Toggle **Pack .atlas
  folders** in the Package dialog.
