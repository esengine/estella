...
is despawned, its references are released — but a released texture isn't
destroyed immediately. It stays resident as an **evictable cache entry**, and if
the same texture is loaded again (re-entering a scene, streaming an area back
in) it is **revived in place** instead of re-fetched and re-decoded. A
byte-budgeted LRU evicts the oldest unreferenced textures once resident VRAM
exceeds the budget. You generally don't manage this by hand — reference and
despawn entities normally.

The budget defaults to **64 MB** (`RuntimeConfig.textureCacheBudget`, also
settable as `textureCacheBudget` in the build config). Size it for your game
with `setTextureBudget`, and watch residency with `getResourceStats`:
