...
Every target honors it — the WeChat `game.json` `deviceOrientation`, the web and
playable builds' rotate-to-fit prompt (and a best-effort `screen.orientation.lock`),
and the desktop window's aspect. Left unset, it's **derived from the design
resolution's aspect** (wider-or-square ⇒ landscape, taller ⇒ portrait), so a
landscape design ships landscape everywhere with zero configuration.
