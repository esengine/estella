...
- **Tileset editor** — author per-tile **collision** (box / circle / polygon, slope
  presets, one-way / sensor / material modifiers), **terrain** sets for autotiling
  (edge/corner peering or multi-color **corner Wang**), per-tile **probability**
  weights and properties, and **animated tiles** (frame sequences with per-frame
  durations and a live preview).
