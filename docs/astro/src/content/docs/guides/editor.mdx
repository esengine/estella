...
Duplicate, Copy Path, Copy Reference, Show in Explorer, and Delete.
...
- **Tilemap painter** — tools for **brush, erase, rectangle, line, bucket, select,
  ...
  (`H` / `V` / `R` while a paint tool is active). See [Tilemaps](/docs/guides/tilemap/).
- **Tileset editor** — mark per-tile **collision** (box or polygon) and **terrain**
  membership for autotiling.
