...
| **Subpackage** | lazy | `PKG` (purple) | inside the package, loaded on demand | large optional content, [WeChat subpackages](/docs/guides/wechat/) |
...
[Assets → Addressable groups](/docs/guides/assets/#addressable-groups-lazy-loading).)
...
**Build** dialog (**File → Build…**):

![The Build dialog's CDN root (hot-update) field](../../../assets/guides/en/hot-update-build-cdn.png)

*The **CDN root (hot-update)** field in the Build dialog writes the active profile's `remoteRoot` to `asset-groups.json`. Leave it empty to load remote assets same-origin (e.g. for local testing).*
...
| `groups.<name>.alwaysInclude` | `boolean` | Ship the group even when nothing in the build *reaches* it — for assets only your code names (a url, a path, rich-text markup). Off by default. See [Assets](/docs/guides/assets/#shipping-what-only-code-names). |
...
survives. See [Assets → Addressable groups](/docs/guides/assets/#addressable-groups-lazy-loading)
...
- [Assets](/docs/guides/assets/) — references, the manifest, typed loaders, and the group model.
- [Building & Exporting](/docs/guides/build-export/) — where the CDN root and `asset-manifest.json` come from.
- [WeChat MiniGame](/docs/guides/wechat/) — how subpackages map to the delivery groups here.
