...
By default a `Sprite` draws with an **unlit** material and ignores every light. To make
it respond, give it a material whose shader declares the **Lit-2D domain**. In an
`.esshader`, that's one pragma plus a call to the injected lighting helper:
...
#pragma vertex
// … pass the world-space position out as v_worldPos …
#pragma end
...
Compile it and assign the material to the renderer exactly as in
