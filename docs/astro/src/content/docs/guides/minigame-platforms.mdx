...
<Aside type="caution">
  This replaces a capability of **a platform you install yourself**. The built-in
  targets install their own profile on import, so selecting *WeChat* in Package
  Project always gets the stock backends — to ship a modified one, define it as a
  project platform (below) and select that instead. Spreading `wechatProfile`
  also does not carry WeChat's `WXWebAssembly` polyfill, which
