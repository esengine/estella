...
1. **Get the runtime for your target.**
   ...
   - *Android* — the host is still built from the engine sources for now. Dawn and
     QuickJS are multi-GB / separate projects, so they are fetched rather than
     vendored; the one-time recipe is in
...
**On iOS that is the whole export**: with the runtime template installed, the
output folder *is* an Xcode project — the `.xcframework`, the app shell and a
generated `.pbxproj` are written around your content. Open it, pick your signing
team under *Signing & Capabilities*, and Run.

Android is assembled by one more command:

```bash
# writes a signed APK (debug keystore unless --keystore says otherwise)
