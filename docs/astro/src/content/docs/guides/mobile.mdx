...
<Aside type="caution">
  `adb` is currently the only reliable way to get that file. Since Android 11 an
  app's `Android/data` directory is not readable by a file manager or by the
  system Files app — `adb shell` can only do it because that user is in the
  `ext_data_rw` group. So this works for you and for a tester who can plug in a
  cable; a player reporting a problem cannot fetch it themselves yet.
</Aside>
