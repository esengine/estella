...
<Aside type="caution">
  API 24–27 is the least verifiable tier we ship. A hosted emulator below API 28
  may have no Vulkan at all, so CI can report "cannot tell" for those rows rather
  than pass or fail. Test on real hardware before promising an Android 7 build.
</Aside>
