---
title: Particles
description: 2D particle system in ESEngine
---

import { Aside } from '@astrojs/starlight/components';

ESEngine provides a GPU-instanced 2D particle system with a C++ simulation backend. The `ParticlePlugin` is included by the engine by default.
...
```typescript
import {
    defineSystem, addSystem, Commands, Transform,
...
addSystem(defineSystem(
    ...
    (commands) => {
        commands.spawn(
            Transform.bundle({ position: { x: 400, y: 300, z: 0 } }),
            ParticleEmitter.bundle({
                rate: 30,
                lifetimeMin: 0.5,
                lifetimeMax: 1.5,
