...
| Contact Hertz | `120` | Contact stiffness (cycles/sec). Higher = less overlap but more jitter |
| Contact Damping Ratio | `10` | Contact bounciness damping. Lower = faster overlap recovery but more energetic |
| Contact Speed | `10` | Max overlap resolution speed (m/s) |
...
- **BoxCollider** — `halfExtents` (Vec2), `offset` (Vec2), `density`, `friction`, `restitution`, `isSensor`, `enabled`
- **CircleCollider** — `radius`, `offset` (Vec2), `density`, `friction`, `restitution`, `isSensor`, `enabled`
- **CapsuleCollider** — `radius`, `halfHeight`, `offset` (Vec2), `density`, `friction`, `restitution`, `isSensor`, `enabled`
...
- [Tilemap](/microes/guides/tilemap/) — tile-based collision generation
