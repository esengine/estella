...
<Aside type="note">
If `build()` throws an exception, the plugin is automatically rolled back — it is removed from the installed plugin set and will not receive `finish()` or `cleanup()` calls. This prevents half-initialized plugins from corrupting engine state.
</Aside>
