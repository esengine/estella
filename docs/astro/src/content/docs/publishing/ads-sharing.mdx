...
context, so it may **not import `esengine`** — the export fails if it does,
which is better than finding out on a device. Inside, you have a 2D canvas from
`wx.getSharedCanvas()`, `wx.getFriendCloudStorage()`, and `wx.onMessage()`
carrying whatever `show()` sent.

### At your desk

Play mode has no context at all, so the board rehearses: the same renderer that
ships inside the context, drawn on an offscreen canvas with obviously-invented
friends. The panel can be laid out and styled without packaging anything.
