...
| **Desktop** | A runnable **native** app per installed desktop template — a `.app` on macOS, a `<Name>/` directory with `<Name>.exe` on Windows. No Chromium and no npm project. | `dist-desktop/` | Double-click it. Selecting the **Steam** channel also writes the depot scripts and a `STEAM.md` with this build's own values — see [Steam](/docs/publishing/steam/). |
