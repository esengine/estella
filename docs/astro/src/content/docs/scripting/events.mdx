...
[a state machine](/docs/gameplay/ai/overview/) — the editor Estella already has for graphs.
...
[prefabs](/docs/world/prefabs/) like any other component — no new asset type:
...
| Action | What it does |
|---|---|
| `ui.setPage` | Switch a [UI controller](/docs/ui/controllers/) to a page. |
...
| `timeline.play` · `timeline.pause` | Drive a [timeline](/docs/animation/timeline/) on the target. |
...
which is what navigation, tabs and dialogs do. Every node in the subtree is told, so "when this
screen appears" is a row on the screen rather than a system that watches for it:

```
shown ─▶ fsm.fire(enter)  on "MissionPanel"   →   Hidden ──enter──▶ Entering
```

The watching costs nothing until something listens for one of the two, and nothing again on a
frame where no UI moved.

[Physics](/docs/gameplay/physics/) contacts reach the same channel (`Physics2DEventType`), so a
[trigger area](/docs/gameplay/markers/) is wired exactly like a button — both participants get
...
[state machine](/docs/gameplay/ai/overview/) decides what it means:
