...
[Transforms, Units & Coordinates](/docs/core-concepts/transforms/).
...
`Transform.rotation` is a quaternion; in 2D you only ever rotate about Z, and a
handful of exports cover the angle ↔ quaternion round trip:

```typescript
import { quat, quaternionToAngle2D } from 'esengine';
...
[Transforms, Units & Coordinates](/docs/core-concepts/transforms/#rotating-in-2d).
...
- [Transforms, Units & Coordinates](/docs/core-concepts/transforms/) — what the
  numbers mean: world units = design pixels, physics meters, Y-up, and rotating
  entities with quaternions.
- [Physics](/docs/gameplay/physics/) — the meter-based unit surface these helpers
  feed into.
