...
![翻页书编辑器——4 帧行走片段,带逐帧时长与实时预览](@/assets/guides/zh/flipbook.png)
...
片段携带**帧事件**(`SpriteAnimEvent`——`{ frame, name, data? }`),播放到某帧时触发:
...
1. 两条路:**右键一张纹理**选**创建精灵动画**——Flipbook 编辑器打开,精灵表上
   覆盖切片网格(格子尺寸按图片猜测;在工具栏调整格宽/格高/边距/间距);或者
   **选中多张图片**,选**用 N 张图片创建精灵动画**,按文件名顺序排好
   (按数字比较,所以 `run_2` 排在 `run_10` 前面)。
2. **点击或拖选格子**追加帧,或**把图片拖到时间轴上**插入。时间轴上一个块有多宽
   就代表这帧有多长:拖它的右边缘改时长,拖块本身重排。视口把当前帧按真实尺寸
   画在剪辑锚点上,相邻帧则各就各位地叠印在后面。
...
**播放头所在的那一帧**在检视器里有自己的一节——时长、尺寸、偏移、锚点,
和剪辑的帧率、循环模式以及下面这两个开关并排。

### 从 Aseprite 文件导入

把 `.aseprite`(或 `.ase`)拖进项目,导入会替你写好产物:合成后的精灵表 `.png` 落在
它旁边,每个 tag 一个 `.esanim` —— `hero_walk.esanim`、`hero_jump.esanim` ——
带着每帧自己的时长与 tag 声明的播放方向。没有重复次数的 tag 会循环,数着播放次数的
不循环。没有 tag 的文件得到一个覆盖全部帧的剪辑。

合成出来的就是画师看到的那一张:所有他们留着可见的图层,连同各自的混合模式与不透明度,
而被关掉的组里的东西一概不进来。tilemap 图层是从 tileset 画出来的而不是像素,导入会
明确报出来,而不是让它悄悄变成空白。

精灵表的纹理导入设置到达时就是 **nearest** 过滤 + 关闭压缩 —— 像素画永远想要、
却从来不是默认值的两项 —— 并且格子尺寸已经填好,`Sprite` 可以直接选一帧。这两项改了
就保持你改的样子;格子则每次导入都重新推导,因为它是源文件的画布尺寸而不是一项偏好。

编辑源文件并保存会原地重新导入:uuid 不变、引用不变、像素是新的。右键 →
**重新导入**可以随时手动触发。

### 逐帧尺寸与偏移

精灵表的格子都一样大,所以整段剪辑共用一个 `Sprite.size` 是对的。多张独立
图片不是:96×64 的攻击帧和 48×96 的待机帧按同一个尺寸画,两个都是错的。
勾上**逐帧尺寸**,播放期间 `Sprite.size` 就归剪辑管——每帧按自己的自然尺寸
(纹理像素,或所在格子)绘制,也可以给某帧写一个 `size` 覆盖。

这同时解锁了**偏移**。偏移把某帧的画面挪开,单位与尺寸相同,加载时被折进
该帧的锚点——所以下游没有任何一层需要认识这个字段。用单位而不是直接改锚点
来表达位移,正是关键:同一个偏移在任意尺寸的帧上都是同样的视觉位移,
归一化的锚点做不到这点。在视口里拖动这一帧即可设定,也可以在检视器里输入。

```jsonc
// step.esanim —— 三张尺寸不同的图片,其中一张往右挪
{
  "version": "1.5",
  "fps": 6,
  "frameSizing": "frame",
...
两者都没声明的剪辑会原封不动地保留实体自己设的 `Sprite.size`,所以在这个能力
出现之前写的剪辑播放起来完全一样。取消勾选会清除该模式以及所有逐帧尺寸与偏移,
回到这种「不插手」的行为。

### 逐帧锚点

锚点就是精灵的落点、旋转与缩放中心——也就是 `Sprite.pivot`,在帧内归一化,
`(0,0)` 为左下角、`(0.5,0.5)` 为居中。当画面在格子里逐帧位移时,只有一个
实体级锚点会让角色跟着滑来滑去;**逐帧**设锚点则能把必须钉住的部位钉住——
脚踩在地面上、手握在武器上。

在 Flipbook 编辑器预览条勾上**锚点**,当前帧上会出现一个十字准星:拖动它
(或在旁边直接输入 X / Y)即可为该帧设锚点。准星的辅助线横贯整个预览台,
配合洋葱皮就能把每一帧的脚与相邻帧对齐。单独设过锚点的帧在帧条上带一个
琥珀色圆点,**×** 清除该覆盖、让此帧回落到剪辑锚点——剪辑锚点在检视器里
就是**默认锚点**。

解析顺序:帧自己的锚点 → 剪辑锚点 → 居中。**完全没有**声明锚点的剪辑会
原封不动地保留实体自己设的 `Sprite.pivot`,所以在你勾上这个开关之前,
老项目一律不受影响;取消勾选会清除剪辑锚点和所有逐帧覆盖,回到这种
「不插手」的行为。

```jsonc
// walk.esanim —— 整段剪辑站在脚上,其中一帧向后仰
{
  "version": "1.4",
  "fps": 10,
  "pivot": { "x": 0.5, "y": 0 },
...
锚点只在剪辑播放某一帧时归剪辑管:`Transform` 或补间照常移动实体,互不干扰。
...
| `createAnimClipFromTextures(textures)` | 一个全新的逐帧贴图剪辑——没有精灵表,且一开始就接管尺寸。 |
| `parseAnimClipData(name, data, textureHandles, textureSizes?)` | 把资产数据解析成运行时 `SpriteAnimClip`——每帧得到纹理句柄、UV 窗口,以及(剪辑接管尺寸时)它的绘制尺寸。传 `textureSizes` 好让逐帧贴图有自然尺寸可回落。 |
...
| `animClipFramePivot(data, frame, size?)` | 该帧实际渲染用的锚点——帧自己的 → 剪辑的 → 居中;给了 `size` 时把该帧的偏移折进去。剪辑没有声明锚点时返回 `null`(表示不要动 `Sprite.pivot`)。 |
| `animClipDrivesPivot(data)` | 该剪辑是否声明了锚点。 |
| `animClipFrameSize(data, frame, natural?)` | 该帧的绘制尺寸——帧自己的 → 你传入的自然尺寸。剪辑不接管尺寸时返回 `null`(表示不要动 `Sprite.size`)。 |
| `animClipDrivesSize(data)` | 该剪辑是否接管 `Sprite.size`——直接设定,或由任一帧的尺寸/偏移隐含。 |

### 用状态机驱动剪辑——零代码

[FSM/BT 内置](/docs/zh-cn/gameplay/ai/overview/)以数据驱动翻页动画。动作的可选**参数**
...
![每种 EasingType 从 0 到 1 的曲线](@/assets/guides/tween-easings.svg)
...
![动画控制器编辑器——Idle / Move / Hop 状态,带条件与参数面板](@/assets/guides/zh/animator-graph.png)

*动画控制器:状态(Idle / Move / Hop)、带条件的转移(`speed>20`、`hop` 触发器),以及驱动它们的**参数**面板。*
...
图编辑器——带位置的状态、带条件的转移,以及**参数**面板——再用资产选择器把 `Animator`
...
控制器可以同时驱动精灵**和** [Spine](/docs/zh-cn/animation/spine/) 动画,所以一套状态机就能
管一整个角色。
...
![一维 blend 按 speed 阈值在 idle / walk / run 之间选择](@/assets/guides/blend-tree.svg)
...
`{ animation, loop? }`):进入时控制器设置该 [Spine](/docs/zh-cn/animation/spine/)
...
剪辑可以给自己内部的时刻命名。`.estimeline` 里的 `customEvent` 轨道携带
`{ time, name, payload }`，动画器在**跨过**该时刻的那一帧报告它——而不是正好落在
它上面的那一帧，固定步长根本不会正好落上去。事件走 `AnimatorEvent` 总线：
...
| 字段 | 说明 |
|---|---|
| `entity` | 被播放动画的实体——事情发生在谁身上。 |
| `name` | 剪辑声明的事件名。 |
| `value` | 数值负载（`payload.value`）；剪辑没写时为 `0`。 |
| `text` | 字符串负载（`payload.text`）；剪辑没写时为空串。 |

动画器给出的承诺：

- 事件按一步覆盖的区间 `(上一时刻, 当前时刻]` 触发，所以再长的一帧也不会跨过去
  漏掉，也不会为同一次跨越报两次。
- 循环回绕算**两个**区间——本圈的尾巴，再加下一圈的开头——所以贴着两端的事件每圈
  正好触发一次。
- 一步内跨过多个事件，按剪辑声明的顺序触发。
- 交叉淡化期间只有**正在进入**的状态报告事件。正在淡出的那个仍在走时钟，但什么都
  不说——所以被闪避打断的攻击不会继续把伤害判定打出来。

动画器不调用任何粒子、音频或玩法代码：它只陈述事件，由拥有这些东西的系统来回应。

### 根运动

一段动画如果动的是它被播放在其上的那个实体，它就不是在摆骨架的姿势——它是在说角色
往哪走。这件事由状态声明：
...
动画器于是把该根轨道的 `position` 与 `rotation` 排除在姿势之外——剪辑永远不会自己
移动实体——并把它请求的位移发布到实体的 `AnimatorRootMotion` 组件上：

| 字段 | 说明 |
|---|---|
| `enabled` | 创作项：是否发布这个请求。 |
| `active` | 本帧是否有声明了根运动的状态在驱动角色。 |
| `deltaPosition` | 请求的位移，世界空间。 |
| `deltaRotation` | 请求的转向，角色自身坐标系。 |
| `deltaTime` | 这两个增量覆盖的秒数。 |

<Aside type="caution">
  这些增量是**请求**，不是结果。把它交给真正移动角色的东西，让世界来回答——
  `ThirdPersonController` 正是这么做的：`active` 期间它忽略摇杆，向
  `CharacterController3D` 请求 `deltaPosition / deltaTime`，所以撞墙的闪避会请求
  整段距离而一步也走不了。把 `deltaPosition` 直接写进 `Transform`，等于让动画器
  接管移动，而接管了移动的动画器会穿墙。
</Aside>

`deltaTime` 与增量一起给出，是因为动画器跑在帧时钟上，而角色控制器通常跑在固定步长
上：请用这一对算出速率。直接把位移加上去的消费者，会在有两个固定步的帧里走两倍，在
没有固定步的帧里一步不走。

判断依据请读 `active`，而不是「增量是否非零」——闪避中间有静止的一瞬，摇杆不能在那
一刻把控制权抢回去。实体上没有 `AnimatorRootMotion` 组件时，该状态依然拒绝去摆角色
自身的 transform，剪辑只是原地播放。
...
- [Spine 动画](/docs/zh-cn/animation/spine/) —— 骨骼动画,也能被状态机驱动。
- [时间轴](/docs/zh-cn/animation/timeline/) —— 关键帧过场与序列化事件。
- [相机](/docs/zh-cn/graphics/camera/) —— 补间 `CameraOrthoSize` 做平滑缩放。
