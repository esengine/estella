...
// stats.retainedBytes         — 为扛过 GPU 丢失而保留的 CPU 副本(见下)
...
图形上下文丢失时(驱动重置、手机浏览器回收 GPU),引擎会在游戏持有的句柄背后重建每张纹理和每个
缓冲。大多数内容从它的来处回来——资产重新加载、文字页按字形重绘、骨骼图集页重新解码。没有这种
来处的内容(运行时构建的网格、加载时拼合的瓦片集)会保留一份 CPU 副本,它占你的内存,却不出现在
任何 GPU 数字里。`stats.retainedBytes` 就是这份副本;引擎自身约 28 KB。副本超过
`RuntimeConfig.retainedBudget`(默认 **16 MB**,构建配置里的 `retainedBudget`,或
`setRetainedBudget(bytes)`;0 表示从不告警)时会告警一次。

引擎放不回来的是**你的游戏自己画的**内容:渲染纹理是 GPU 存储,设备重建后交还给你的是空的。
`onDeviceRestored` 告诉你什么时候该重画,`RenderTexture.contentLost` 告诉你哪一张需要重画——
每帧都被相机渲染的那种从来不需要。
...
onDeviceRestored(() => {
  ...
  drawMinimap(minimap);                  // 你自己的绘制
  RenderTexture.contentDrawn(minimap);   // ……并声明它又有内容了
});
```
