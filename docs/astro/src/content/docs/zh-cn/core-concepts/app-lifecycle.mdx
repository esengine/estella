...
| `randomSeed` | `number` | — | 给引擎的随机性播种,让这次运行可复现——回放、bug 复现、对某一帧下断言。不填则每次运行都不同,这对粒子正是玩家想要的。各子系统取的是按名字独立的流,所以新增一个发射器不会改变别处抽到的数。 |
...
物理**不在**默认集合里:`Physics2DPlugin` 需要物理 wasm 模块。发行运行时会在项目
声明了物理、或场景含有物理组件时,自动从侧模块 host 获取 `'physics'` 并安装插件;
手动组装时请自己构造 `Physics2DPlugin`(它和 `loadPhysicsModule` 都从 `esengine`
...
这里的资源是纯数据,为的是让例子聚焦在插件的形状上——但资源同样可以是一个带方法的类
实例,引擎的每个子系统都是这么发布自己 API 的。见
[插件与资源](/docs/zh-cn/core-concepts/plugins-resources/)。
...
| `app.wasmModule` | 已连接的 `ESEngineModule`,`connectCpp` 之前为 `null`。属于**嵌入**契约——见下文。 |
...
`connectCpp`、`app.wasmModule` 和 `world.getCppRegistry()` 属于与本页其余部分不同的
契约:它们是**宿主**——编辑器、打包后的游戏外壳、自定义运行时——把引擎内核绑定到
应用上的方式。游戏本身从不调用它们,因为拿到手的 App 已经连好了。

正因如此它们被标记为 `@internal`,而它们所使用的类型(`ESEngineModule`、`CppRegistry`、
`CppResourceManager`)从 `esengine/wasm` 导出,而不是 `esengine`。写宿主就从那里导入;
写游戏则完全不需要写出这些名字。
...
| `app.waitForPhysics()` / `app.isPhysicsReady` | 等待 / 查询异步的物理 wasm 初始化(没装 `Physics2DPlugin` 时为 no-op / `false`)。 |
...
<Aside type="note">
  主 `esengine` barrel 恰好镜像了物理/Spine 的**接线**层——`Physics2DPlugin`、
  `Physics2DEvents`、`Physics`、`loadPhysicsModule`、`SpinePlugin`、
