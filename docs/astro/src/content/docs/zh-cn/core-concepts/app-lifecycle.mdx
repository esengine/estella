...
`lifecyclePlugin(options?)`(两个工厂都默认安装)发布 **`Lifecycle`** 资源——
...
| `autoPause` | `boolean` | 页面隐藏时是否暂停应用。默认 `true`;运行时可写,初值来自 `lifecyclePlugin({ autoPause })`。 |
...
- **前置声明 `colorSpace` 和 `backend`。**两者都在创建时固化(着色器编译、设备
  注入)——不能在存活应用上切换。
- **别和生命周期自动暂停较劲。**用 `lifecyclePlugin({ autoPause: false })`(或
