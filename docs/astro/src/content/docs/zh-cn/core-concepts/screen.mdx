...
- 世界是 **Y 向上**;屏幕坐标是 **Y 向下**(由
  [`CameraView`](/docs/zh-cn/graphics/camera/) 为你转换)。
...
相机适配与 UI 布局盒走的是**同一个**顺序,所以任一时刻只有一个设计分辨率在起作用——
场景的 `Canvas` 即使还留着比项目适配更旧的分辨率,也不会出现"编辑器里一个盒子、发行版
里另一个盒子"。
...
适配开启时它就是**权威**——相机和 UI 布局盒都一样,因为两者解析适配源的方式相同。
...
编辑器改为从适配源恢复*固定的*设计分辨率盒,并在那里布局 UI——布局在任何缩放级别都完全
...
这个盒子既有**大小**(UI 在多大的区域内布局),也有**位置**(这块区域落在世界的什么地方)。
位置决定了 HUD 会不会跟着相机走:

- **没有父实体**的 UI 根是*屏幕根*:引擎每帧把它放在盒子中心。运行时这个中心就是相机,
  所以相机滚多远,HUD 都留在屏幕上。编辑器里盒子锚定在世界原点(`EDITOR_UI_ANCHOR`),
  也正是设计框覆盖层绘制的位置——平移缩放时 UI 和其他内容一起动。
- **挂在某个实体下**的 UI 根是*世界空间 UI*:它跟随父级的 transform(单位头顶的血条、
  墙上的面板),盒子只负责给它定尺寸。

所以"让这个 HUD 跟随相机"不需要做任何事——那就是默认行为。反过来要把 UI 钉在世界里,
给它的根加一个父级即可。

### 哪些相机能看见:`Camera.cullingMask`

每个可绘制物都属于**一个排序层**,而一个相机渲染其中的一个集合。`Camera.cullingMask`
就是这个集合的位掩码——第 *i* 位 = 第 *i* 层,默认全开,所以从没配置过的相机和以前
画得一模一样。

UI 同样属于某一层,但**不是**通过那个给它排序的字段:UI 绘制的 `layer` 是它在树里的
次序(`1000 + 树序),这正是 0–31 的层遮罩够不着它的原因。它的**归属**改由
`Canvas.layer` 表达——一个普通的排序层,不影响任何绘制顺序,存在的唯一目的就是让相机
能包含或排除这个画布的 UI。

小地图因此才成立:把 HUD 的 `Canvas` 放到单独一层,然后在小地图相机上清掉那一位。
...
编辑器里这个遮罩是一个按**项目排序层名字**列出的多选框(项目设置 → 渲染 → 层名称)。
编辑器自己的视口不受它影响、永远显示所有层,所以剔除不会在创作时挡住内容。

<Aside type="caution">
  子视口相机只清除自己那块矩形,但它的**绘制并不会被裁剪到该矩形内**——它画的东西可能
  铺满整帧。剔除决定这类相机**画什么**,限定**画在哪**是另一个尚未合上的口子。
</Aside>
...
- [相机](/docs/zh-cn/graphics/camera/) —— orthoSize、跟随、混合、分屏。
- [UI](/docs/zh-cn/ui/overview/) —— 消费设计像素盒的 flexbox 布局。
- [物理](/docs/zh-cn/gameplay/physics/) —— `pixelsPerUnit` 真正起作用的地方。
- [编辑器](/docs/zh-cn/editor/overview/) —— 显示设置页与设备预览。
