...
| `CharacterController.velocity`、`MotorJoint.linearVelocity` | 世界像素/秒。 |
...
`WeldJoint`、`WheelJoint`、`MotorJoint`),或命令式创建一个并驱动它的马达 / 限位:
...
给静态(或运动学)碰撞体加 `OneWayPlatform`,使其**单面**——刚体从一个方向落上去、从其它
