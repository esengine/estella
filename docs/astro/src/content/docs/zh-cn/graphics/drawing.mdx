---
title: 自定义绘制
description: 超出组件的绘制——用 Draw 画即时模式图元、用 Graphics 画矢量路径、用 MeshRenderer 与 Geometry API 画自定义三角网格,以及离屏渲染纹理。
---
...
| `ShapeRenderer` / `MeshRenderer` | **组件**——带排序层、材质、编辑器 gizmo 的实体。 | 属于场景的形状/网格(见[精灵](/docs/zh-cn/graphics/sprites/))。 |
...
`MeshRenderer` 组件渲染任意的索引三角形列表——程序化地形切片、径向血条弧、水面。
...
`MeshRenderers` 资源——`setGeometry(entity, geometry)` 重新上传(索引在引擎侧校验),
另有 `getGeometry` / `clearGeometry`。只改 `geometry` 字段本身不会被变更检测。

## 自定义几何:`Geometry` API

`MeshRenderer` 是*组件*路径,`Geometry` 则是*即时*路径:它上传一个索引顶点缓冲,交回
...
要把**整个场景**画进去,让一台相机指向它:把 `Camera.renderTarget` 设成目标的句柄,
那台相机就画到那里而不是屏幕——在屏幕相机之前渲染,也不参与「谁是主视图」的竞争。
它看见什么由自己的排序层(`Camera.cullingMask`)决定,所以小地图相机和玩家相机可以
看同一个世界、显示不同的部分。目前仅 WebGL2:WebGPU 后端还不能把渲染目标的纹理
交给 `Sprite`:
