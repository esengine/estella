---
title: 精灵与渲染
description: 用 Sprite 组件绘制 2D 图像——纹理、着色与透明、尺寸与轴心、翻转、排序层与绘制顺序、Y 排序与深度层(2.5D)、视差——外加纹理过滤、点击检测、位图缓存、ShapeRenderer 图元、精灵滤镜、拖尾与世界空间位图文字。
---

import { Aside } from '@astrojs/starlight/components';

**`Sprite`** 是 2D 渲染的主力:一个带 `Sprite` 组件的实体在世界里绘制一个带纹理的四边形。要绘制
不带纹理的实心图元,有 **`ShapeRenderer`**。一切都经由 [相机](/docs/zh-cn/graphics/camera/) 绘制,
而绘制顺序由**排序层**控制。

## 你的第一个精灵

一个精灵需要 `Transform`(在哪)和 `Sprite`(画什么)。通过 [`Assets`](/docs/zh-cn/assets/overview/)
资源加载一张纹理,把它的句柄放到精灵上:
...
| `pivot` | `{0.5,0.5}` | 精灵旋转/缩放所绕的轴心点,取值是相对 `size` 的比例。超出 0–1 就落在精灵外面。 |
| `layer` | `0` | 排序层——控制绘制顺序(见下)。 |
| `flipX` / `flipY` | `false` | 水平 / 垂直镜像。 |
| `material` | `0` | 自定义[材质](/docs/zh-cn/graphics/materials/)句柄(`0` = 默认精灵着色器)。 |
...
`color`(和 `size`)都**可动画**——在 [Sequencer](/docs/zh-cn/animation/timeline/) 里给它们打关键帧
做闪烁和淡入淡出。
...
轴心超出 `0–1` 是合法且有用的:`{0.5, -1}` 把转动点放到画面下方整整一个精灵高度处——
摆动的手臂、挂在自己画面之外那根轴上的门,就是这么做的。

存成比例才能让轴心在改尺寸后依然成立,但没人是这么想事情的——**按像素输入**、
**从九个常用位置里点一个**、或者**直接在精灵上拖**,见[在编辑器里](#在编辑器里)。
...
<Aside type="note">
  ...
  多少设计像素等于一个 Box2D 米(见[物理](/docs/zh-cn/gameplay/physics/))。精灵的
...
命名是为了可读性,不是层数上限:你可以命名最多三十二个槽位(槽位的下标**就是**它的 z 序)。
名字只是建议——直接敲一个列表里没有的数字,字段照样接受;一个名字都不填时,`layer` 就是
普通数字输入框。两种情况下渲染器都按任意整数排序。
...
<Aside type="tip">
  排序层做粗粒度分组(背景 vs 玩法 vs 世界内 UI)。层**内**的精细排序,声明 `sprite.order` —— 见下文。
</Aside>

### 层内次序

`sprite.order` 是精灵在其排序层**内部**的位置:越大越靠上,范围 −128…127,`0` 表示什么都没说。
它压过该层本来会推断出的顺序 —— 画家层的 z、Y 排序层的世界 Y:

```typescript
shadow.order = -1;  // 始终在主人脚下,无论主人走到哪
weapon.order = 1;   // 始终在握住它的手上面
```

在 Y 排序层上这是**唯一**的覆盖手段,因为 z 根本进不了那把键。没有它的时候,想让影子留在
主人脚下,只能挪动精灵的 Y —— 也就是谎报它站在哪里。

### 排序组

一个 order 是这一个精灵对**整层其他所有东西**的承诺,而两个拼装角色一重叠,这个承诺就不够用了:
把武器抬到另一个角色之上,也就把它抬到了自己身体之上。

`SortingGroup` 让一个实体连同它的整棵子树**作为一个单元**参与排序。组声明整个装配体对外呈现的
层与次序;每个成员自己的 `sprite.order` 变成它在**组内**的位置,而组外的任何东西都不可能落到
两个成员之间。

```typescript
world.insert(character, SortingGroup, { layer: 2, order: 5 });
// 在角色内部,这两个数现在只需要表达"哪个部件在前"。
world.update(body, Sprite, (s) => { s.order = 0; });
world.update(arm, Sprite, (s) => { s.order = 1; });
```

于是两个角色之间只由各自的组来排序,谁的部件都不会和对方的交错。

组可以嵌套。一个自带 `SortingGroup` 的武器预制体挂到手臂下面,无论手臂走到哪都保持为一整块 ——
外层组仍然拥有那个层,而这一块不会被手臂的某个兄弟节点拆开。

<Aside type="caution">
  组的成员,和一个恰好与该组**层与次序都相同**的组外精灵,两者之间的交错是未定义的 ——
  "没有东西落在成员之间"是关于这个组自己那个位置的承诺,所以请给组一个属于它自己的位置。
</Aside>

### 精灵遮罩

`SpriteMask` 把一个实体**自己的精灵**变成模板(stencil):在它之后绘制、并且主动要求被遮罩的精灵,
会被它的形状裁切。精灵本身什么都不用改 —— 纹理、尺寸、轴心、翻转、9 宫格都是你已经创作好的那些 ——
而在它正在遮罩期间,遮罩自己的精灵不会被画出来。它是那道裁切,不是一张图。

```typescript
world.insert(hole, SpriteMask, { alphaCutoff: 0.5 });
world.update(fog, Sprite, (s) => {
  s.maskInteraction = SpriteMaskInteraction.VisibleOutside;  // 在迷雾上挖一个洞
});
```

遮罩是**逐精灵主动加入**的(`Sprite.maskInteraction`),所以往场景里加一个遮罩,永远不会让无关的精灵消失:

| `maskInteraction` | 保留哪一部分 |
|---|---|
| `None`(默认) | 忽略遮罩。 |
| `VisibleInside` | 只保留盖在遮罩上的部分 —— 一扇窗。 |
| `VisibleOutside` | 只保留避开遮罩的部分 —— 一个洞。 |

`alphaCutoff` 大于 0 时按精灵的**形状**裁切而不是它的方框:低于该 alpha 的片元不参与遮罩,
所以圆形遮罩裁出的是圆。

遮罩**向前生效** —— 它裁切在它之后绘制的东西,也就是你已经用层和层内次序安排好的那个顺序。
那正是它的模板被写入的时刻,所以与遮罩处在**同层同次序**的精灵完全不会被裁。打开 `limitRange`
可以让它的作用范围停在指定的层与次序上。

<Aside type="caution">
  两个遮罩重叠的地方,由**更近的那个**说了算 —— 它写在后面,模板里留下的是它。被更远那个遮罩裁切的
  精灵会恰好在更近的遮罩内部消失,所以任何一个精灵都只由一个遮罩来裁。
</Aside>

遮罩裁的是画面,不是场景:拾取、物理和空间查询都看不见它。
...
在 **Project Settings → Rendering → Depth-sorted layers** 勾选某个层后,该层完全不再依赖
绘制顺序:谁遮住谁由深度缓冲判定,依据是各实体 `Transform` 的 z。配合透视相机就是 2.5D——
不同深度的精灵在任意角度下都能正确互相遮挡,无需手工维护排序。

哪些精灵参与,由其材质的**混合模式**决定,因为它正说明了这次绘制要不要读取屏幕上已有的内容:

| 混合模式 | 在深度层中 |
| --- | --- |
| `None`(不透明) | 写入深度,并被任何更近的内容遮挡,与绘制先后无关。 |
| 其它(半透明) | 只测试深度、从不写入,仍按由远及近的绘制顺序。 |

这个区分不是一个可选项——半透明精灵若写入深度,会把它背后的精灵裁出生硬的黑边。所以想要正确
遮挡的精灵需要不透明材质;不动混合模式则一切照旧,只是前面的东西会挡住它。

编辑时要看到这些,切换视口工具栏的 **2D / 3D** 按钮——编辑器自己的眼睛变成透视的。它只改变
你的视角:Game 视图始终显示场景相机。拾取和拖拽会跟着走,所以 z = -400 的精灵在它被画出的
位置就能抓到。

<Aside type="note">
  一个层要么 Y 排序、要么深度排序,不会两者兼有——Y 排序本就是从世界 Y 投影出的深度,
  同时声明两者的层仍按 Y 排序。可用 z 范围在正交相机下是 ±`far`(默认 ±1000),
  透视相机下是 `near`..`far`;超出范围的内容会被整个裁掉。
</Aside>
...
- **图集**:把 `uvOffset` 和 `uvScale` 设为某个单元格的归一化矩形,显示打包图集里的一格。(逐帧动画
  请用 [Animator](/docs/zh-cn/animation/overview/) 驱动这些,而非手动。)
...
要让精灵响应 `Light` 光源,最简单的路径是设 `sprite.lit = true`——无需自定义材质
(用平面法线受光)。需要法线贴图或完全自定义的观感时,改用 **Lit-2D 材质**。两种方式都见
[2D 光照与阴影](/docs/zh-cn/graphics/lighting/)。
...
[UI](/docs/zh-cn/ui/overview/)。

## 精灵点击检测

"这次点击落在这个精灵上了吗?"分两步:把屏幕点转到世界空间,再对精灵的矩形做测试。
第一步由 `CameraView` 资源完成(见[相机](/docs/zh-cn/graphics/camera/));第二步 SDK
导出了感知轴心的点测试:
...
不带深度缓冲的 [RenderTexture](/docs/zh-cn/graphics/drawing/)。
...
- **`pivot` 有三种授权方式**,写进去的都是同一个比例值:
  - 字段下方的 **3×3 网格**,一次点击选中九个常用位置之一。
  - 该行上的 **`比例` / `px`** 切换按像素输入:`px` 显示 `pivot × size`,输入 `32` 存进去的是
    `32 / size`。单位只是视图,场景里始终存比例。
  - 用**指针工具**时,单选中的精灵会在视口里得到一个**轴心点**。拖它会让轴心在画面内移动
    而**画面本身不动**(位移由 Transform 补偿),于是你可以直接用眼睛把转动点放到脚下或
    门轴上。它在移动/旋转/缩放工具下会隐藏——那几个工具的中心抓手正好在同一个位置。

## 参见

- [纹理](/docs/zh-cn/assets/textures/) —— 图片本身：格式、导入设置，以及怎么读它的宽高。
- [动画](/docs/zh-cn/animation/overview/) —— 精灵表帧动画与属性补间。
- [自定义绘制](/docs/zh-cn/graphics/drawing/) —— 精灵之外的网格与即时模式图形。
- [2D 光照与阴影](/docs/zh-cn/graphics/lighting/) —— 法线贴图与逐精灵光照。
