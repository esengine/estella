...
- [脚本](/docs/zh-cn/guides/scripting/) —— 行为是声明式系统;叶子就是普通函数。
- [动画](/docs/zh-cn/guides/animation/) —— 用状态机驱动精灵动画。
- [场景](/docs/zh-cn/guides/scene/) —— `.esfsm` / `.esbt` 资产随场景加载与序列化。
- [事件绑定](/docs/zh-cn/guides/events/) —— 点击或触发可以 `fsm.fire` 进状态机；动作与条件出自同一个注册表。
