...
但释放的纹理不会立即销毁:它以**可逐出缓存条目**的形式留驻,同一纹理再次被加载(重进场景、
流式加载重新进入某区域)时会被**原地复活**,而不是重新下载和解码。一个按字节预算的 LRU
在常驻显存超过预算时逐出最旧的未引用纹理。你一般不用手动管理——正常引用和销毁实体即可。

预算默认 **64 MB**(`RuntimeConfig.textureCacheBudget`,也可在构建配置里用
`textureCacheBudget` 设置)。用 `setTextureBudget` 按你的游戏调大小,用
`getResourceStats` 观测驻留情况:
