...
[热更新](/docs/zh-cn/guides/hot-update/)能在已发行的游戏里替换它。手写的 `fetch` 一样都拿不到——
...
见[音频指南](/docs/zh-cn/guides/audio/))。
...
想逐帧观察这些数字(编辑器内和运行时),见[性能分析](/docs/zh-cn/guides/profiling/)。
逐纹理的**采样/过滤**参数(像素风 nearest 还是平滑 linear)见
[精灵指南](/docs/zh-cn/guides/sprites/)。
