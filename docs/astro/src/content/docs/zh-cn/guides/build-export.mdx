...
| **Android** | **原生** arm64 应用(Vulkan)的内容——真 APK,不是 WebView。 | `dist-android/` | `node build-tools/cli.js native --package --content dist-android` 把它包成已签名的 APK。见 [Android 与 iOS](/docs/zh-cn/guides/mobile/)。 |
| **iOS** | **原生** arm64 应用(Metal)的内容;若本机已为 iOS 构建过引擎,还会附带 Xcode 工程。 | `dist-ios/` | 打开生成的 Xcode 工程,选签名 Team,Run。见 [Android 与 iOS](/docs/zh-cn/guides/mobile/)。 |
...
<Aside type="caution" title="移动端需要引擎源码检出">
  ...
  [Android 与 iOS](/docs/zh-cn/guides/mobile/)。
</Aside>
...
**高级**里放着[热更新](/docs/zh-cn/guides/hot-update/)的 CDN 根地址、source map,
以及构建结束后是否打开输出目录。
...
<Aside type="caution" title="只有代码点名的资产会被剔除">
  ...
  [资源](/docs/zh-cn/guides/assets/#只有代码点名的资产)。
</Aside>
...
- **CDN(不进包)** —— `remote` 组里的资产。它们是真实存在的字节,但住在你的 CDN 上,对包体零成本。见[热更新](/docs/zh-cn/guides/hot-update/)。
...
<Aside type="note">
  ...
  编辑器和 SDK 里。例外是[移动端](/docs/zh-cn/guides/mobile/):在那里,围绕导出组装应用
  也由这个 CLI 完成。
</Aside>

## 参见

- [Android 与 iOS](/docs/zh-cn/guides/mobile/) —— 原生应用:前置条件、如何组装,以及只在真机上才咬人的那些事。
