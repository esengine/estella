...
<Aside type="caution">
  `color` 可以填,但目前还没有作用到 DragonBones 骨架上——运行时的 ABI 没有着色入口。
  在 DragonBones Pro 里给插槽设的颜色**是**生效的;这里说的是叠在它之上的那层每实体着色。
</Aside>
...
| `setEntityProps(entity, props)` | 一次设 `{ skeletonScale?, flipX?, flipY?, layer?, playing?, timeScale? }`。 |
