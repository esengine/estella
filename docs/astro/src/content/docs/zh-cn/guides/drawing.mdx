...
| `ShapeRenderer` / `Mesh2D` | **组件**——带排序层、材质、编辑器 gizmo 的实体。 | 属于场景的形状/网格(见[精灵](/docs/zh-cn/guides/sprites/))。 |
...
一个**句柄**由你自己绘制——配上[材质](/docs/zh-cn/guides/material/)用
...
小地图、头像、镜面——以及[位图缓存](/docs/zh-cn/guides/sprites/)的底层——
`RenderTexture` 分配一个离屏渲染目标。把绘制命令包在 `begin`/`end` 之间即渲染
进去;得到的 `textureId` 是普通纹理句柄,哪里都能用(`Sprite.texture`、
`Draw.texture`、材质纹理参数):
...
| 方法 | 作用 |
|---|---|
| `create(options)` | 分配目标;返回带 `textureId`、`width`、`height` 的句柄。 |
| `begin(rt, viewProjection)` | 把后续绘制重定向进目标(矩阵由你提供)。 |
| `end()` | 结束离屏渲染,回到屏幕。 |
| `resize(rt, width, height)` | 释放并重建——返回**新**句柄(`textureId` 会变)。 |
