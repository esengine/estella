...
把控制权交给[状态机](/docs/zh-cn/guides/ai/)的动作——Estella 已经有图编辑器了。
...
一根线就是 `EventBinding` 组件，因此它会序列化进场景、随[预制体](/docs/zh-cn/guides/prefab/)
一起走，和任何组件一样——不新增资产类型：
...
| 动作 | 作用 |
|---|---|
| `ui.setPage` | 把 [UI 控制器](/docs/zh-cn/guides/ui-controllers/)切到某一页。 |
...
| `timeline.play` · `timeline.pause` | 驱动目标上的[时间轴](/docs/zh-cn/guides/timeline/)。 |
...
所以入场动画是挂在那块面板上的一行，而不是一个每帧去盯着的系统：

| 事件 | 动作 | 目标 |
|---|---|---|
| `shown` | `tween.play` | *（本实体）* |

在没有人监听这两个事件之前，这份「盯着」不花任何代价；有人监听、但这一帧 UI 没动过时，同样不花。

[物理](/docs/zh-cn/guides/physics/)接触也走同一条通道（`PhysicsEventType`），所以
[触发区](/docs/zh-cn/guides/markers/)的接线方式和按钮一模一样——双方都会收到事件，各自拿到对方：
...
突破口——点击发出一个触发器，实体的[状态机](/docs/zh-cn/guides/ai/)来决定它意味着什么：
