...
哪里生成玩家?”。给同一个实体加上一个静态 [`RigidBody`](/docs/zh-cn/guides/physics/) 和一个
...
- 它**可 `Query`**——同一条代码路径既读你手工放置的标记,*也*读从
  [Tiled](/docs/zh-cn/guides/tilemap/) `.tmj` 对象层导入的标记。
...
碰撞体,因此需要[物理](/docs/zh-cn/guides/physics/):在编辑器和 Web 运行时,当场景用到物理
...
[碰撞体 gizmo](/docs/zh-cn/guides/physics/) 来塑形。
...
[碰撞层](/docs/zh-cn/guides/physics/#碰撞过滤)上,让每个传感器的掩码收窄它要考虑的范围。完整
事件面见 [碰撞与传感事件](/docs/zh-cn/guides/physics/#碰撞与传感事件)。

## 从 Tiled 导入

当 [`Tilemap`](/docs/zh-cn/guides/tilemap/) 加载一张 `.tmj` 地图时,它的**对象层**汇聚到你
...
- **Trigger Area 是传感器**——要*挡住*的东西(墙、台沿),请画一块
  [碰撞层](/docs/zh-cn/guides/tilemap/#碰撞障碍层)或用非传感器碰撞体。

## 参见

- [瓦片地图](/docs/zh-cn/guides/tilemap/) —— 碰撞(障碍)层与喂给同一批 Marker 的 Tiled 对象层。
- [物理](/docs/zh-cn/guides/physics/) —— 传感器、碰撞过滤,以及 Trigger Area 背后的 `PhysicsEvents` 资源。
- [场景](/docs/zh-cn/guides/scene/) —— Marker 与 Trigger Area 随场景序列化。
- [事件绑定](/docs/zh-cn/guides/events/) —— 有东西进入 Trigger Area 时执行一个动作，直接挂在触发区上，不写代码。
