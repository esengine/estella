...
<Aside type="caution">
  这替换的是**你自己安装的平台**的能力。内置目标在 import 时就装好了自己的 profile，
  所以在打包对话框里选「微信小游戏」拿到的永远是原装后端——要发布改过的版本，
  请把它定义成一个项目平台（见下）并改选它。另外展开 `wechatProfile` 不会带上
  `initWeChatPlatform()` 装的 `WXWebAssembly` polyfill，走这条路要自己调
  `esengine/wechat` 的 `polyfillWebAssembly()`。
</Aside>
