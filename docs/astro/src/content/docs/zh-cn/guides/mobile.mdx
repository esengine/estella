...
1. **把原生宿主构建一次。** Dawn 和 QuickJS 体量数 GB、又是独立项目,所以是按需拉取而不是
   vendored 进来。一次性的构建配方(以及确切的 `cmake` 调用)见
...
2. **装好平台工具链。**
   ...
   - *iOS* —— **装有 Xcode 的 macOS**。Apple 不为其他操作系统提供工具链,
     所以对话框直说,而不是假装可以。
...
```bash
# Android —— 写出已签名的 APK(未用 --keystore 指定时用 debug keystore)
node build-tools/cli.js native --package --content dist-android

# iOS(在 Mac 上)—— 围绕内容写出 Xcode 工程
node build-tools/cli.js native --target ios --package --content dist-ios
```

<Aside type="tip" title="iOS:导出可以直接帮你写出 Xcode 工程">
  当引擎已经在同一台机器上为 iOS 构建过时,**导出本身**就会生成 Xcode 工程——生成它不需要
  编译器(引擎是预构建的 `.xcframework`,应用外壳只是一个 `main.m` 加一份 plist),
  所以它属于导出流程,而不该藏在一条你得自己找到的命令后面。打开它,选好签名 Team,Run。
  如果这台机器从未为 iOS 构建过引擎,导出仍会写出内容,并告诉你哪条命令能补上缺的那块。
</Aside>
