...
的 `velocity`;系统会解析一次运动学 move-and-slide,把结果写回,并报告它碰到了什么:
...
| `maxSlides` | number | `4` | 每次移动的滑动迭代次数(拐角需要更多)。 |
| `skinWidth` | number | `1` | 与表面保持的间隙(px),避免粘住。 |
| `snapLength` | number | `0` | 向下探测(px)以贴住斜坡/台阶;`0` = 关。 |
| `slideOnCeiling` | boolean | `true` | 沿天花板滑动 vs 停止移动。 |
