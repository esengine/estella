...
`UIGear`),会随 [预制体](/docs/guides/prefab/) 一起走、序列化进场景。它是
[按钮](/docs/guides/ui-components/#button) 那种"单控件状态"的共享化、多页化推广——
...
[时间轴](/docs/guides/timeline/) 属性轨道一致——组件名 + 点路径 `property`
...
一个 `Interactable` + 一条绑定,它就重现出按钮状态,不需要 `StateMachine`——这是对旧状态组件的
统一替代:
...
上面的一切都能可视化授权,无需写码。选中一个 UI 根,**控制器** 面板(在 UI 模式下弹出)
列出它的控制器:

- **添加控制器**,再增删 **页**;页选择器切换 `current`,视口实时重排。
- 点击某个控制器使其 **活动**。在 [细节](/docs/guides/editor/) 面板里,每个字段旁便出现一个小的
  **gear 点**——点它把该字段绑定到活动控制器(以当前页 + 字段当前值作种子)。
- 打开 **录制**:此时编辑一个被绑定的字段,值就写入控制器的 **当前页**。切页、改、切页、改——
  每页各自记录自己的值。这是快捷路径——"选一页,随便改,自动录制"。
...
**从信号** —— `bindControllerPage` 用一个 [响应式信号](/docs/guides/ui/) 驱动控制器的页,
这是数据驱动页签的声明式路径:
...
[FSM / 行为树](/docs/guides/ai/)(`.esfsm` / `.esbt`)无需写码就切页。参数是 `"controller:page"`:
...
- [UI](/docs/guides/ui/) —— 绑定所驱动的 flexbox 组件模型。
- [UI 控件](/docs/guides/ui-components/) —— 控件工厂。
- [时间轴](/docs/guides/timeline/) —— 关键帧过渡;绑定用的是同一套字段寻址。
- [游戏 AI](/docs/guides/ai/) —— 能调用 `ui.setPage` 的 `.esfsm` / `.esbt`。
