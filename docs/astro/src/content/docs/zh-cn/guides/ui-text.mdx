...
`fontFamily` 在字形栅格化时由平台的 **Canvas2D** 解析——宿主页面能解析的任何
字体族都可用:系统字体,或页面自己加载的 Web 字体(CSS `@font-face` 或
`FontFace` API)。SDK 不会替 `Text` 组件抓取字体文件;请确保 Web 字体在文本
首次渲染前已加载,否则其字形会用回退字体栅格化。
