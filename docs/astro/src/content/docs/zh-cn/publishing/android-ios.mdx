...
<Aside type="note">
  Vulkan 的要求来自这套运行时:引擎经 Dawn 渲染,而 Dawn 的 Android 后端就是 Vulkan。
  打包后的游戏里没有 GLES 退路。它写在清单里,所以 Play 会据此过滤,而不是让不具备
  条件的设备装上再失败。
</Aside>
...
见[资源](/docs/zh-cn/assets/overview/#只有代码点名的资产)。

### 为内存最紧张的平台单独调纹理

逐资产的**导入设置**每个平台一个页签,移动目标也在其中——所以一张纹理可以为手机压得比
网页端更狠,而不用动源 PNG。见[资源](/docs/zh-cn/assets/overview/)。
...
<Aside type="caution">
  这条路径是给你的,不是给玩家的:Android 11 起,一个应用的 `Android/data` 对文件管理器
  和系统「文件」应用都是关的,`adb shell` 能读只是因为那个用户恰好在 `ext_data_rw` 组里。
</Aside>

**游戏崩溃后,下一次启动会把记录发布到玩家能拿到的地方**——
...
所以给报障的人的说法是:**再打开一次游戏,然后把那个目录里最新的 `estella-crash-*.log` 发过来**。
...
- [构建与导出](/docs/zh-cn/publishing/overview/) —— 所有目标、cook 选项与打包对话框。
- [资源](/docs/zh-cn/assets/overview/) —— 可达性剔除、交付分组与导入设置。
