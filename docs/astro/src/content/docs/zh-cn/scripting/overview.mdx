...
<Aside type="caution">
  **引擎组件拿到的是拷贝。** 对*脚本*组件——你自己 `defineComponent` 出来的,以及 `ctx.self`
  ——`ctx.get` 给的就是存储对象本身,改了就生效。而 `Transform`、`Sprite`、`RigidBody2D` 这些
  C++ 支撑的组件,每次读都是从 wasm 堆里解出来的一份拷贝,所以
  `ctx.get(Transform).position.x += …` 改的是一个没人保存的值,而且**静默无效**。
  读出来、改完、用 `ctx.set` 交回去。
</Aside>
...
你的脚本由 esbuild 打包,而它会从项目自己的 `node_modules` 解析依赖——所以装一个库就是
普通的安装:
...
它会**随游戏发到每一个目标**——网页与桌面构建、小游戏包(并降级到该宿主接受的语法)、
原生内容负载,以及编辑器自己的 Play。没有额外步骤,也没有逐平台配置:你 import 了什么,
打包进去的就是什么。
...
**任何需要 Node 的东西。** 游戏跑在浏览器或小游戏宿主里,两者都没有 `fs`、`crypto`、
...
如果这个 import 是你自己写的,改用 Web API(`crypto.subtle`、`fetch`、`localStorage`)
或引擎自己的等价物——文件用 [资产](/docs/zh-cn/assets/overview/)、玩家数据用
[存档](/docs/zh-cn/scripting/save/),它们在所有平台都能用。如果伸手的是**某个依赖**,那修法在
包那一侧:很多包都发布了浏览器安全的入口——上面的 `protobufjs/minimal` 正是——还有些带
`browser` 字段,esbuild 会自动选用。

**而且它要占包体。** 依赖是被整个打进去的,而这在上限最紧的地方最要命:微信把小游戏主包
限制在 4MB。构建完成后,[体积报告](/docs/zh-cn/publishing/overview/#包体体积)会把包体拆开给你
看——涨上去的库会出现在**脚本**那一类里。

<Aside type="tip">
  优先选明确支持浏览器的包。为 Node 写的库要么直接让构建失败(这其实是好事——你立刻就
  知道了),要么拖进来一堆 polyfill,在 4MB 的预算下那本身就是个问题。
</Aside>
...
| `ctx.input` | `InputState` | 键盘 / 鼠标 / 触摸 / 手柄(见 [输入](/docs/zh-cn/scripting/input/))。 |
...
带 `RigidBody2D` 的实体会被跳过——物理求解器拥有它们的变换,两者永远不会争抢同一个实体。
...
钩子签名——`entity`、`world`、`commands`,外加 `self`——也和[游戏 AI](/docs/zh-cn/gameplay/ai/overview/) 层
...
- [输入](/docs/zh-cn/scripting/input/) —— 在系统里读取键盘、指针与手势。
