...
`UIGear`),会随 [预制体](/docs/zh-cn/guides/prefab/) 一起走、序列化进场景。它是
[按钮](/docs/zh-cn/guides/ui-components/#button) 那种"单控件状态"的共享化、多页化推广——
...
[时间轴](/docs/zh-cn/guides/timeline/) 属性轨道一致——组件名 + 点路径 `property`
...
- **添加控制器**(指针按钮预设一键添加带四页的 `$interaction`),页以 **chip** 管理:
  ...
  绑定,删页会清掉该页已录的值。
- 点击某个控制器使其 **活动**。在 [细节](/docs/zh-cn/guides/editor/) 面板里,每个字段旁便出现一个小的
...
**从信号** —— `bindControllerPage` 用一个 [响应式信号](/docs/zh-cn/guides/ui/) 驱动控制器的页,
这是数据驱动页签的声明式路径:
...
**从按钮，且不写代码** —— 在按钮上挂一根[事件连线](/docs/zh-cn/guides/events/)，点击时执行
...
[FSM / 行为树](/docs/zh-cn/guides/ai/)(`.esfsm` / `.esbt`)无需写码就切页。参数是 `"controller:page"`:
...
- [UI](/docs/zh-cn/guides/ui/) —— 绑定所驱动的 flexbox 组件模型。
- [UI 控件](/docs/zh-cn/guides/ui-components/) —— 控件工厂。
- [时间轴](/docs/zh-cn/guides/timeline/) —— 关键帧过渡;绑定用的是同一套字段寻址。
- [游戏 AI](/docs/zh-cn/guides/ai/) —— 能调用 `ui.setPage` 的 `.esfsm` / `.esbt`。
- [事件绑定](/docs/zh-cn/guides/events/) —— 把点击直接接到 `ui.setPage`，不写代码。
