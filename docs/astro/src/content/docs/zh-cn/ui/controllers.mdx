...
[按钮](/docs/zh-cn/ui/widgets/#button-按钮) 那种"单控件状态"的共享化、多页化推广——
...
[FSM / 行为树](/docs/zh-cn/gameplay/ai/overview/)(`.esfsm` / `.esbt`)无需写码就切页。参数是 `"controller:page"`:
...
- [游戏 AI](/docs/zh-cn/gameplay/ai/overview/) —— 能调用 `ui.setPage` 的 `.esfsm` / `.esbt`。
