...
`ReadonlySignal<string>` 驱动 [UI 控制器](/docs/zh-cn/ui/controllers/)
...
[ListView](/docs/zh-cn/ui/lists/) 本身*已经*是数据驱动的——它渲染自
...
| 有限集合中的一个**命名状态**——页签、开/关、按钮状态 | [控制器 + gear](/docs/zh-cn/ui/controllers/) | 一次翻页带动多元素重排;可在编辑器创作;支持补间。用 `bindControllerPage` 从数据接入。 |
| 一个**集合**——背包行、排行榜 | `DataSource` + [ListView](/docs/zh-cn/ui/lists/) | 细粒度变更通知驱动虚拟化。 |
| 由模拟派生的**逐帧**值——冷却扫过、跟随玩家的标记 | 普通[系统](/docs/zh-cn/scripting/overview/)写组件 | 值本来每帧都变;query 是天然的读者。 |
...
- [UI](/docs/zh-cn/ui/overview/) —— 绑定所写入的组件模型。
- [UI 控件](/docs/zh-cn/ui/widgets/) —— 值控件(`createSlider`、`createToggle`、`createDropdown`)。
- [UI 控制器](/docs/zh-cn/ui/controllers/) —— 命名的页状态 + gear;`bindControllerPage` 把信号接进去。
- [UI 列表](/docs/zh-cn/ui/lists/) —— `DataSource` 驱动的虚拟化列表。
- [主题](/docs/zh-cn/ui/theming/) —— 设计令牌与实时换肤。
