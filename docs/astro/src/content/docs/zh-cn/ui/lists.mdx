...
在 Estella 里,长的可滚动内容由 [UI](/docs/zh-cn/ui/overview/) 组件模型之上的两个工厂构建:
...
- [UI](/docs/zh-cn/ui/overview/) —— 这些工厂所依托的组件模型。
- [UI 控件](/docs/zh-cn/ui/widgets/) —— 其余控件工厂(Button、Dialog、文本输入……)。
- [UI 布局](/docs/zh-cn/ui/layout/) —— `UINode` 盒模型、flexbox 与尺寸。
- [UI 数据绑定](/docs/zh-cn/ui/data-binding/) —— 信号与控件双向绑定。
- [UI 交互](/docs/zh-cn/ui/interaction/) —— 指针事件、焦点、拖放。
- [UI 主题](/docs/zh-cn/ui/theming/) —— 滚动条与默认色所取用的 token 角色。
