...
| `overflow` | `TextOverflow` | `Visible` | `Visible` / `Clip` / `Ellipsis`。当前渲染器始终按 `Visible` 绘制;要裁剪溢出,给盒加 [`UIMask`](/docs/zh-cn/ui/overview/)。 |
...
| `strokeWidth` | number | `0` | 描边宽度(px);`0` 关闭。SDF 图集上是把字形自己的边缘向外推,所以任意宽度下描出来的仍是这个字的形状([见下文](#描边))。 |
| `shadowColor` | Color | `{0,0,0,1}` | 投影颜色。 |
| `shadowOffsetX` / `shadowOffsetY` | number | `0` | 投影偏移(px)。 |
| `shadowBlur` | number | `0` | 投影柔度(px)。`0` 是硬边偏移副本;大于 0 时投影摊成一圈,读起来是字下面一团柔和的暗,而不是又盖了一遍字。只给柔度不给偏移就是一圈光晕。 |
...
`strokeWidth` + `strokeColor` 给字形描边,`shadow*` 在其后投影。两者都从填充所用的
同一张图集绘制,所以带样式的标签仍与不带样式的合批。

**描边是把字形的边缘推出去,不是围一圈副本。** SDF 字形自带到自身边缘的距离,
描边就是把这条边缘向外推:同样的四边形画出同一个形状,只是长大了。你给的宽度会
经图集的 spread 与渲染尺寸换算,因此在任何 `fontSize`、任何相机缩放下含义一致;
要得比图集能膨胀的更宽时,退化为最宽的那圈真实描边,而不是糊满字形的格子。

**Bitmap** 字形没有距离可推,仍走旧的 8 方向字形拓印。那在发丝宽时读起来像描边,
超过几个像素就糊成一团——要更粗的描边,请强制
[`renderMode: Sdf`](#双字形管线)。

`shadowBlur` 大于 0 时,投影在中心加外围一圈共九次拓印,每次的 alpha 从合成方程里
反解出来,使叠起来正好是你要的 alpha。与描边不同,它是把边缘**变柔**而不是挪动,
这也是两者分成两个字段的原因。
...
| `<img src="…" width=24 height=24 />` | 内联图片——随文本一起排布的图标(见[下文](#内联图片))。 |
...
offsetY=N scale=N tint=#RRGGBB />`(必须自闭合,`src` 必填)让一张图片随文本内联
排布。摆放字形的那套富文本布局同时摆放图片盒——`valign` 决定它在行内如何对齐,
`width`/`height`/`scale` 定尺寸,`offsetX/Y` 微调位置,`tint` 重新着色——最终作为
`Text` 下的一个子图片 quad 渲染。`src` 经 `Assets.loadTexture` 解析,因此是项目
相对的纹理路径。

<Aside type="note">
  内联图片在**不换行**的富文本上渲染(把 `Text` 的 `wordWrap` 设为 `false`);
  换行的富文本目前只排文本,会丢弃图片片段。内联图片还要求 `Text` 有 `UINode`
  盒子——无盒子的世界空间标签上画不出来。
</Aside>
...
没有 Localization 资源时按原作 `content` 显示。见[本地化](/docs/zh-cn/assets/localization/)。
...
- [UI 布局](/docs/zh-cn/ui/layout/) —— 文本对齐与换行所依的 `UINode` 盒。
- [UI 组件](/docs/zh-cn/ui/widgets/) —— 可编辑文本的 `createTextInput`。
- [UI 主题](/docs/zh-cn/ui/theming/) —— 主题化的文本颜色角色与字号阶梯。
- [UI 数据绑定](/docs/zh-cn/ui/data-binding/) —— 用响应式 signal 驱动 `content`。
- [本地化](/docs/zh-cn/assets/localization/) —— 目录、语言与 `i18nKey` 解析。
