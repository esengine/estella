...
所以活下来的实体拥有自己正在绘制的东西——就是它当前绑定的那一个实例,哪怕文件后来被热更替换
过——并在实体销毁时(包括作为父节点子树的一部分被销毁)交回。它也会留在原地:被一个没能存活
...
- **组件按「引用」而非运行时句柄命名的资源**(例如动画剪辑)。它们仍随场景一起释放,所以要
  拿一份比场景活得更久的引用:在引导时 `Assets.preload`,或者用一个永不卸载的叠加场景。
  纹理、材质、字体、网格与环境是按句柄绑定的,会跟着走。
