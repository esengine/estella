...
它**会带走**的是:自己绑定的资源。提升会把场景的那份引用**分裂**出一份,而不是转移或丢弃,
所以活下来的实体拥有自己正在使用的东西,并在实体销毁时(包括作为父节点子树的一部分被销毁)
交回。对贴图、材质、字体、网格、环境来说,那就是它当前绑定的那一个实例,哪怕文件后来被热更
替换过;对动画剪辑、时间轴、状态机、行为树、动画控制器——组件按引用命名、每帧查表的那些——
带走的是那个条目本身,所以实体会继续跟随它的更新。它也会留在原地:被一个没能存活
的父节点摘下来的实体,停在它原本所在的位置,而不会跳到「相对父节点」的那个坐标上。

有两样东西**不会**跟着走:

- **场景的系统**。场景声明的每个系统都会在卸载时注销,所以持久化实体的行为必须来自注册在
  `App` 上的系统,而不是场景 `systems` 列表里的。
- **没有任何地方声明过的资源**。提升读的是引擎其它部分同样在读的 `assetFields`,所以你自己
  加载、又赋值到未声明字段上的贴图它看不见——那种要拿一份比场景活得更久的引用:在引导时
  `Assets.preload`,或者用一个永不卸载的叠加场景。
- **它那些没有被标记的子节点**。「这个实体持久化」说的是这个实体,不是它的子树:没有自己被
  标记的子节点会随场景销毁;而一个持久化的子节点若父节点不持久,会被摘下来单独存活。
