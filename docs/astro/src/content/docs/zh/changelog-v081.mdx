---
title: v0.8.1
description: ESEngine v0.8.1 更新日志
---
...
新增 Tiled 地图加载器和瓦片地图渲染管线，完整编辑器集成。

- **Tiled JSON 加载器** — 加载 `.tmj` / `.json` 格式的 Tiled 地图，自动解析 tileset 纹理
...
- **`PhysicsDebugDraw`** — 物理碰撞体、关节和接触点的可视化叠加层，编辑器和运行时均可使用
- 渲染 Box、Circle 和 Capsule 碰撞体的线框轮廓
