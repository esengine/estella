...
| 方法 | 返回值 | 说明 |
|------|--------|------|
| `fetchJson<T>(path)` | `Promise<T>` | 加载并解析 JSON 文件 |
| `fetchText(path)` | `Promise<string>` | 加载文本文件 |
| `fetchBinary(path)` | `Promise<ArrayBuffer>` | 加载二进制文件 |
...
类型化加载方法（`loadTexture`、`loadSpine`、`loadMaterial`、`loadPrefab`）按路径缓存。同一路径加载两次会直接返回缓存结果，不会再次发起网络请求。通用获取方法（`fetchJson`、`fetchText`、`fetchBinary`）不缓存——每次调用都从后端获取。
...
对于 Playable Ad 构建，编辑器的构建流程会自动将所有引用的资源以 data URI 形式嵌入输出包中。运行时 `assets.loadTexture()` / `assets.fetchJson()` 等加载调用会直接从嵌入数据解析，无需网络请求，也无需修改任何代码。
...
插件可以注册自定义资产类型处理器，使场景加载器能够发现和加载你的资产类型：
...
内置处理器（texture、material、font、spine、audio、tilemap、timeline、anim-clip）会自动注册。自定义处理器通常在插件的 `build()` 方法中注册。
