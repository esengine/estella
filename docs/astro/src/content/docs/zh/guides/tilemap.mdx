...
1. 在 **Hierarchy** 面板中右键 → **创建** → **Tilemap**
2. 在创建对话框中设置瓦片大小，选择 **有限** 或 **无限** 地图模式
3. 在 **Tilemap** 组件 Inspector 中分配 tileset 图片
...
1. 在 Hierarchy 中右键 → **创建** → **Tilemap**（参见上方[编辑器瓦片地图创建](#编辑器瓦片地图创建)）
...
<Aside type="tip">
  导入地图时，在 [Tiled Map Editor](https://www.mapeditor.org/) 中设计地图，导出为 JSON（`.tmj`），将文件和 tileset 图片放在项目资源文件夹中。
</Aside>
