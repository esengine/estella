...
A gallery of the engine's `ParticleEmitter`. Page through six composed effects
...
`Snowfall` (a wide, slow drift) · `Sparkles` (a twinkling field). Each mixes
shape, velocity, colour/size-over-life, gravity, spin and blend mode — see
`src/config.ts`.
...
src/
  ...
  config.ts               # the six showcases, the firework, spawnShowcase()
