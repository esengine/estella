...
import { Physics2D, MotorJoint2D } from 'esengine/physics';
import type { Physics2DAPI } from 'esengine/physics';
...
export const dragSystem = defineSystem(
    [Res(Input), Res(UICameraInfo), Res(Physics2D), Query(RigidBody2D)],
    (input, camera: UICameraData, physics: Physics2DAPI, bodies) => {
...
export const shuttleSystem = defineSystem(
    [Res(Physics2D), Query(Transform, MotorJoint2D)],
    (physics: Physics2DAPI, shuttles) => {
