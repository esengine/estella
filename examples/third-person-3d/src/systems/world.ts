 ...
 * Sent as a blow rather than subtracted here — `applyDamage` is the only writer
 * of `Health`, so a fire and a sword cost the runner the same way.
 */
export const hazardSystem = defineSystem(
    [Res(Time), Res(Run), Query(Transform, Runner), Query(Hazard, Transform), EventWriter(Damage)],
    (time: TimeData, run: RunData, runners, hazards, damage) => {
        if (run.phase !== 'playing') return;
        for (const [runner, at] of runners) {
            for (const [, zone, origin] of hazards) {
                ...
                damage.send({
                    target: runner as Entity, source: runner as Entity,
                    amount: zone.damagePerSecond * time.delta,
                    x: at.position.x, y: at.position.y, z: at.position.z,
                });
            }
        }
    },
