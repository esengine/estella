 ...
 */
export type BuiltinPlatform = 'web' | 'desktop' | 'wechat' | 'douyin' | 'kuaishou' | 'bilibili' | 'quickgame' | 'playable' | 'android' | 'ios';

/** Built-ins, in the order they are offered. */
export const BUILTIN_PLATFORMS: readonly BuiltinPlatform[] = [
    'web', 'desktop', 'wechat', 'douyin', 'kuaishou', 'bilibili', 'quickgame', 'playable', 'android', 'ios',
];
 ...
 */
export const MINIGAME_PLATFORMS = ['wechat', 'douyin', 'kuaishou', 'bilibili', 'quickgame'] as const;
 ...
 * CommonJS mini-game one, or that one again without wasm SIMD and BigInt i64 for
 * the quick-game hosts, whose V8 (8.3) refuses both.
 */
export function engineBuildFor(platform: ExportPlatform): 'web' | 'minigame' | 'quickgame' {
    if (platform === 'quickgame') return 'quickgame';
...
export function compileTargetFor(platform: ExportPlatform): 'wasm' | 'native' | null {
    if (platform === 'web' || isMiniGamePlatform(platform)) return 'wasm';
