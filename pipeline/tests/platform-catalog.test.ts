...
describe('createProjectPlatform — scaffolding', () => {
  it('writes both halves, already joined, and the result is discoverable', async () => {
    ...
    expect(loaded!.profile.emitConfigFiles({ title: 'T', appid: '', version: '1.0.0', orientation: 'portrait', subPackages: [], includeSuffixes: [], hasOpenData: false, openDataRoot: 'open-data' })[0].file)
      .toBe('game.json');
  });
...
describe('loadProjectPlatform — the profile handed to exportMiniGame', () => {
  it('merges over the family defaults, so a minimal profile is complete', async () => {
    ...
    expect(p.emitConfigFiles({ title: 'T', appid: '', version: '1.0.0', orientation: 'portrait', subPackages: [], includeSuffixes: [], hasOpenData: false, openDataRoot: 'open-data' }))
