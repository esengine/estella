...
/** The editor version the probes run as. A native runtime template is matched
 *  against it exactly, so the tests own both ends of that comparison. */
const VERSION = '9.9.9';

const dirs = () => ({ web: webDir, minigame: wxDir, quickgame: path.join(wxDir, '..', 'qg') });
...
beforeEach(() => {
  ...
  // The native probes read the machine's template store; point it at this test's
  // own empty one, or the result would depend on what the developer has installed.
...
describe('createProjectPlatform — scaffolding', () => {
  it('writes both halves, already joined, and the result is discoverable', async () => {
    ...
    expect(loaded!.profile.emitConfigFiles({ title: 'T', appid: '', version: '1.0.0', versionCode: 1, icon: null, orientation: 'portrait', subPackages: [], includeSuffixes: [], hasOpenData: false, openDataRoot: 'open-data' })[0].file)
      .toBe('game.json');
  });
...
describe('loadProjectPlatform — the profile handed to exportMiniGame', () => {
  it('merges over the family defaults, so a minimal profile is complete', async () => {
    ...
    expect(p.emitConfigFiles({ title: 'T', appid: '', version: '1.0.0', versionCode: 1, icon: null, orientation: 'portrait', subPackages: [], includeSuffixes: [], hasOpenData: false, openDataRoot: 'open-data' }))
