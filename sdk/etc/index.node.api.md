...
Symbols: 1083 stable · 50 beta · 1 deprecated
...
## InterestPolicy — type @beta
```
(view: InterestView) => ReadonlySet<Entity> | 'all'
```

## InterestView — interface @beta
```
...
## MessageHandler — type @beta
```
(payload: T) => void
```

## MessagePortLike — interface @beta
```
...
## MessagePortTransport — class @beta
```
close: () => void
onMessage: ((data: string | ArrayBuffer) => void) | null
send: (data: string | ArrayBuffer) => void
...
## NetChannelOptions — interface @beta
```
...
## NetRoleKind — type @beta
```
'offline' | 'server' | 'client'
```

## NetSession — class @beta
```
...
## REPLICATION_CHANNEL — const @beta
```
1
```

## REPLICATION_PROTOCOL_VERSION — const @beta
```
1
```

## RadiusInterestOptions — interface @beta
```
...
## ReplicatedData — interface @beta
```
netId: number
owner: number
```

## ReplicationClient — class @beta
```
applyPending: () => void
...
## createMemoryTransportPair — function @beta
```
