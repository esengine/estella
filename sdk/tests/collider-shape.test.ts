 ...
 * pre-refactor Physics2DDebugDraw math exactly (this is the Phase-0 parity proof) so both
 * the runtime debug draw and the editor gizmo can render from one source.
 */
import { describe, it, expect } from 'vitest';
import {
    shapeCenter, collider2DOutline, CAPSULE_ARC_SEGMENTS,
    type Collider2DShape,
} from '../src/physics/ColliderShape2D';
...
describe('shapeCenter', () => {
    it('adds the collider offset (metres × ppu), unrotated', () => {
        const box: Collider2DShape = { kind: 'box', halfExtents: { x: 0.5, y: 0.5 }, offset: { x: 1, y: 0 } };
    ...
    it('is the entity position for offsetless shapes (segment/polygon/chain)', () => {
        const seg: Collider2DShape = { kind: 'segment', point1: { x: -0.5, y: 0 }, point2: { x: 0.5, y: 0 } };
        near(shapeCenter(seg, { x: 7, y: 9 }, 1.234, 100), 7, 9);
    });
});

describe('collider2DOutline', () => {
    it('box → a closed 4-corner loop, ppu-scaled', () => {
        const box: Collider2DShape = { kind: 'box', halfExtents: { x: 0.5, y: 0.5 }, offset: { x: 0, y: 0 } };
        const o = collider2DOutline(box, { x: 0, y: 0 }, 0, 100);
    ...
    it('box corners match the legacy drawRotatedBox math under rotation', () => {
        ...
        const box: Collider2DShape = { kind: 'box', halfExtents: { x: hx / 100, y: hy / 100 }, offset: { x: 0, y: 0 } };
        const p = collider2DOutline(box, { x: cx, y: cy }, angle, 100).polylines[0];
    ...
    it('circle → a single {c,r}, no polylines', () => {
        const c: Collider2DShape = { kind: 'circle', radius: 0.5, offset: { x: 0, y: 0 } };
        const o = collider2DOutline(c, { x: 5, y: 5 }, 0, 100);
    ...
    it('capsule → one closed loop with the same segment count the legacy draw emitted', () => {
        const cap: Collider2DShape = { kind: 'capsule', radius: 0.25, halfHeight: 0.5, offset: { x: 0, y: 0 } };
        const o = collider2DOutline(cap, { x: 0, y: 0 }, 0, 100);
        expect(o.circles).toEqual([]);
    ...
    it('segment → one open two-point polyline', () => {
        const seg: Collider2DShape = { kind: 'segment', point1: { x: -0.5, y: 0 }, point2: { x: 0.5, y: 0 } };
        const o = collider2DOutline(seg, { x: 0, y: 0 }, 0, 100);
        expect(o.polylines).toHaveLength(1);
    ...
    it('polygon → a closed loop of n+1 points', () => {
        const poly: Collider2DShape = { kind: 'polygon', vertices: [{ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 0, y: 1 }] };
        const p = collider2DOutline(poly, { x: 0, y: 0 }, 0, 100).polylines[0];
    ...
    it('chain → open by default, closed when isLoop', () => {
        ...
        const open = collider2DOutline({ kind: 'chain', points: pts, isLoop: false }, { x: 0, y: 0 }, 0, 100);
        expect(open.polylines[0]).toHaveLength(3); // 2 segments
        const loop = collider2DOutline({ kind: 'chain', points: pts, isLoop: true }, { x: 0, y: 0 }, 0, 100);
        expect(loop.polylines[0]).toHaveLength(4); // 3 segments (closed)
    });

    it('degenerate chain (<2 points) draws nothing', () => {
        const o = collider2DOutline({ kind: 'chain', points: [{ x: 0, y: 0 }], isLoop: false }, { x: 0, y: 0 }, 0, 100);
        expect(o.polylines).toEqual([]);
    });
});
