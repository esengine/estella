...
describe('a ref-bound asset is owned as a slot', () => {
    it('the last holder takes the publication and its dependencies with it', async () => {
        ...
        expect(registry.get('walk.esanim')).toEqual({ name: 'anim-clip:walk.esanim#1' });
        ...
        expect(released).toEqual(['anim-clip:walk.esanim#1:texture']);
    ...
    it('a republish swaps what every name resolves to, and retires only the old era', async () => {
        ...
        expect(await slots.republish(kind, 'assets/walk.esanim')).toBe(true);
        // Both names, one slot, one era.
        expect(registry.get('walk.esanim')).toEqual({ name: 'anim-clip:walk.esanim#2' });
        expect(registry.get('assets/walk.esanim')).toEqual({ name: 'anim-clip:walk.esanim#2' });
        expect(released, 'the era it replaced gave back what it held').toEqual(['anim-clip:walk.esanim#1:texture']);

        // The holder never named an era, so it holds the slot exactly as before.
        held.release();
        expect(registry.size).toBe(0);
        expect(released).toEqual(['anim-clip:walk.esanim#1:texture', 'anim-clip:walk.esanim#2:texture']);
    });
    ...
    it('a retiring era cannot unpublish the era that replaced it', async () => {
        ...
        await slots.republish(kind, 'walk.esanim');

        first.release();
        expect(registry.get('walk.esanim'), 'gen1 leaving took gen2 out').toEqual({ name: 'anim-clip:walk.esanim#2' });
        second.release();
    ...
    it('every alias resolves to one slot, not to a copy of the object', async () => {
        ...
        await slots.republish(kind, '@uuid:1');
    ...
    it('a failed republish leaves the holders exactly what they had', async () => {
        ...
        await expect(slots.republish(broken, 'walk.esanim')).rejects.toThrow('no bytes');
        expect(registry.get('walk.esanim')).toEqual({ name: 'anim-clip:walk.esanim#1' });
