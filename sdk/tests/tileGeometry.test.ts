...
import {
    tileCellCenter, tileCellOutline, isNonOrthogonal, TilemapOrientation,
    type TileGridParams,
...
describe('tileCellCenter', () => {
    it('orthogonal: tile (0,0) center sits at (+hw,-hh); +tx→right, +ty→down', () => {
        const p = { ...base, orientation: TilemapOrientation.Orthogonal };
    ...
    it('isometric: tile (0,0) center is the origin; the lattice is a diamond', () => {
        const p = { ...base, orientation: TilemapOrientation.Isometric };
        centerEq(p, 0, 0, 0, 0);
    ...
    it('staggered (axis Y, index odd): odd rows shift right hw and every row drops hh', () => {
        const p = { ...base, orientation: TilemapOrientation.Staggered };
        centerEq(p, 0, 0, 32, -16);
    ...
    it('staggered index even flips which rows shift', () => {
        const p = { ...base, orientation: TilemapOrientation.Staggered, staggerIndexEven: true };
    ...
    it('staggered axis X shifts odd columns down instead of rows right', () => {
        const p = { ...base, orientation: TilemapOrientation.Staggered, staggerAxisX: true };
        // colW = (64+0)/2 = 32
        centerEq(p, 0, 0, 32, -16);
    ...
    it('hexagonal (axis Y): row pitch is (th+side)/2, side defaults to th/2', () => {
        const side8 = { ...base, orientation: TilemapOrientation.Hexagonal, hexSideLength: 8 };
        // rowH = (32+8)/2 = 20
        centerEq(side8, 0, 0, 32, -16);
        centerEq(side8, 0, 1, 64, -36); // odd row shifted +hw, y = -(20 + 16)
        centerEq(side8, 0, 2, 32, -56); // even row, y = -(40 + 16)

        const dflt = { ...base, orientation: TilemapOrientation.Hexagonal }; // side → th/2 = 16
        // rowH = (32+16)/2 = 24
        centerEq(dflt, 0, 2, 32, -64); // y = -(2*24 + 16)
    });

    it('hexagonal axis X: column pitch is (tw+side)/2', () => {
        const p = { ...base, orientation: TilemapOrientation.Hexagonal, hexSideLength: 8, staggerAxisX: true };
        // colW = (64+8)/2 = 36
        centerEq(p, 0, 0, 32, -16);
...
describe('tileCellOutline', () => {
    it('orthogonal outline is the tile rect', () => {
        const o = tileCellOutline({ ...base, orientation: TilemapOrientation.Orthogonal });
    ...
    it('isometric/staggered outline is a diamond (points on the axes)', () => {
        for (const orientation of [TilemapOrientation.Isometric, TilemapOrientation.Staggered]) {
    ...
    it('hexagonal axis Y is a pointy-top hex: 6 verts, flat edges vertical of length side', () => {
        const o = tileCellOutline({ ...base, orientation: TilemapOrientation.Hexagonal, hexSideLength: 8 });
    ...
    it('hexagonal axis X is a flat-top hex: points on the x axis', () => {
        const o = tileCellOutline({ ...base, orientation: TilemapOrientation.Hexagonal, hexSideLength: 8, staggerAxisX: true });
        expect(o).toHaveLength(6);
...
describe('isNonOrthogonal', () => {
    it('is true for iso/staggered/hex, false for orthogonal', () => {
        expect(isNonOrthogonal(TilemapOrientation.Orthogonal)).toBe(false);
        expect(isNonOrthogonal(TilemapOrientation.Isometric)).toBe(true);
        expect(isNonOrthogonal(TilemapOrientation.Staggered)).toBe(true);
        expect(isNonOrthogonal(TilemapOrientation.Hexagonal)).toBe(true);
    });
});
