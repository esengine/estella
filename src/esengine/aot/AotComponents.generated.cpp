...
ComponentAt engineComponentAt(ecs::Registry& registry, const char* name) {
    ...
    if (std::strcmp(name, "MeshLightmap") == 0) {
        return [&registry](std::uint32_t raw) -> void* {
            return registry.tryGet<esengine::ecs::MeshLightmap>(Entity::fromRaw(raw));
        };
    }
    if (std::strcmp(name, "MeshMorph") == 0) {
        return [&registry](std::uint32_t raw) -> void* {
            return registry.tryGet<esengine::ecs::MeshMorph>(Entity::fromRaw(raw));
        };
    }
...
CandidatesOf engineComponentCandidates(ecs::Registry& registry, const char* name) {
    ...
    if (std::strcmp(name, "MeshLightmap") == 0) {
        return [&registry]() -> Candidates {
            return denseAsIds(registry.entitiesWith<esengine::ecs::MeshLightmap>());
        };
    }
    if (std::strcmp(name, "MeshMorph") == 0) {
        return [&registry]() -> Candidates {
            return denseAsIds(registry.entitiesWith<esengine::ecs::MeshMorph>());
        };
    }
    if (std::strcmp(name, "MeshRenderer") == 0) {
        return [&registry]() -> Candidates {
