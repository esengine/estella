...
std::string canonicalVertexStageWGSL(bool lit) {
    ...
    src +=
        ...
        "#endif\n"
        "#ifdef MESH\n"
        // The GLSL twin's morph block. Declaring the texture in the VERTEX stage
        // is what earns it vertex visibility in the bind-group layout.
        ...
        "#endif\n"
        // Before the pose: a target was authored against the mesh at rest.
        ...
        "#elif defined(MESH)\n"
        "    let world = mat4x4f(v.a_model0, v.a_model1, v.a_model2, v.a_model3) * vec4f(local, 1.0);\n"
        "    out.v_color = v.a_color * v.a_instTint;\n"
        "#else\n"
...
std::string canonicalVertexStage(bool lit) {
    std::string src =
        ...
        "#endif\n"
        "#ifdef MESH\n"
        // The shapes a resident mesh is blended towards. See mesh.esshader, whose
        // vertex stage says the same thing to the same blocks: a material's mesh
        // variant deforms the same geometry, so it must deform it the same way.
    ...
    src +=
        ...
        "#elif defined(MESH)\n"
        "    vec4 world = mat4(a_model0, a_model1, a_model2, a_model3) * vec4(local, 1.0);\n"
        "    v_color = a_color * a_instTint;\n"
        "#else\n"
