...
/** Where the chunks begin, and how many of them the header claims. */
struct BlobHeader {
    usize payloadAt;
    u32 chunkCount;
};
...
/**
 * The header, or a refusal. A blob written under another encoding parses
 * perfectly and means other tiles, so it is refused rather than decoded — and
 * the legacy magic carries no header at all, which is why it means v1 and not
 * "whatever this engine is now".
 */
inline bool parseBlobHeader(const u8* raw, usize size, BlobHeader& out) {
    ...
    if (magic == BLOB_MAGIC_V2) {
        ...
        wrote = { fields[0], fields[1], fields[2], fields[3], fields[4] };
    ...
    if (!(wrote == runningEncoding())) return false;
