...
def compute_schema_hash(components: List[Component]) -> str:
    ...
    for comp in sorted(components, key=lambda c: (c.namespace or '', c.name)):
        ...
        for prop in comp.properties:
            annotations = ','.join(sorted(prop.annotations.keys())) if prop.annotations else ''
            h.update(f'{prop.name}:{prop.cpp_type}:{annotations};'.encode('utf-8'))
        h.update(b'\n')
...
class EditorAPIGenerator:
    ...
    def generate(self) -> str:
        ...
        lines.extend(self._gen_set_string())
    ...
    def _gen_header(self) -> List[str]:
        schema_hash = compute_schema_hash(self.components)
        lines = [
            '/**',
    ...
    def _collect_string_fields(self, comp: Component) -> list:
        ...
        for prop in comp.properties:
            ...
            t = self.types.clean_type(prop.cpp_type)
            if t == 'std::string':
                string_fields.append((prop.name, prop.name))
        return string_fields

    def _gen_set_string(self) -> List[str]:
        lines = [
            '// ── Set String Property ──',
            '',
        ...
        for comp in self.components:
            if not comp.properties:
                continue
            full = f'{comp.namespace}::{comp.name}' if comp.namespace else comp.name
            string_fields = self._collect_string_fields(comp)
            ...
            for i, (key, cpp_path) in enumerate(string_fields):
                prefix = 'if' if i == 0 else 'else if'
                lines.append(f'        {prefix} (field == "{key}") {{ c.{cpp_path} = value; }}')
    ...
    def _gen_get_string(self) -> List[str]:
        lines = [
            '// ── Get String Property ──',
            '',
        ...
        for comp in self.components:
            ...
            first_comp = False
            lines.append(f'    {cond} (comp == "{comp.name}") {{')
            lines.append(f'        if (!reg.has<{full}>(entity)) return std::string{{}};')
            lines.append(f'        const auto& c = reg.get<{full}>(entity);')
