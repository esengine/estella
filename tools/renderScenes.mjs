...
export const SCENES = [
  ...
  // The same 3D view head-on, where the eye stands IN the ground it works on: the
  // plane is edge-on through the eye's own point, so it has no picture at all.
  // Counting that hit paints one point's grid coordinate over the whole frame.
  { id: "editor-grid-edge-on", tier: "pr", webgpu: true, env: { ESTELLA_VERIFY_SCENE: "/scenes/mesh-instanced.esscene", ESTELLA_VERIFY_GRID: "64", ESTELLA_VERIFY_GRID_EXPECT: "nothing", ESTELLA_VERIFY_PERSPECTIVE: "1", ESTELLA_VERIFY_ORBIT: "0,0", ESTELLA_VERIFY_STEPS: "3" } },
  ...
  // KNOWN RED ON WEBGPU: a sprite at z = +1 under an ortho camera at z = 0 is inside the
  // symmetric box mat4.ts builds and below 0 of what WebGPU clips to. RenderFrame.cpp:954
  // dodges this for shadows; a camera cannot, and celestial-heights' HealthBar_Fill is there.
